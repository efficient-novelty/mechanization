...
A determinate universe cannot require an external oracle that surveys indefinitely remote futures before each act of becoming. If actuality depended on such a survey, the law of becoming would either be teleological in formulation or computationally opaque in implementation. Time-symmetric and whole-history approaches make vivid why one might be tempted by such a picture, but they also sharpen the need for a local account of articulation \citep{Price1996,WheelerFeynman1945,WheelerFeynman1949}. Any viable Selective Law must therefore be \emph{localizable}: it must rank candidate continuations by features available in the realized present and its immediately accessible continuation cone.
...
The argument is still deliberately schematic. A later section makes the skeleton explicit in a notation compact enough for evaluation while remaining neutral about any later evaluator or metric.
...
\begin{itemize}
    ...
    \item $\preceq_H$ is a context-sensitive preorder on $A(H)$. It records comparative eligibility for actualization relative to $H$, once the local balance of generative affordance and stabilization burden is taken into account.
...
The preorder $\preceq_H$ is kept abstract on purpose. At this level of the paper it does not stand for a single numerical metric or a completed evaluator. It stands for the minimal comparative structure needed to say that one admissible continuation is at least as eligible for actualization as another in context $H$. Using a preorder rather than a total order also leaves room for ties. Two admissible continuations may be equally eligible at this level of abstraction. A later theory may refine such ties by additional local structure, or treat them as a genuine parity that must be handled without appeal to a remote whole-history survey. What matters here is that ``better'' means better relative to the realized history and its admissible continuation cone, not better by inspection of indefinitely distant futures.
...
Here $\oplus$ is only schematic notation for bundling two candidate moves into one composite continuation. The example does not assume any special algebraic structure beyond that intuitive idea of combination.
...
Only $c$ survives every filter that the framework imposes. Constitutive constraint excludes $a$. Selective sufficiency excludes $b$. Primeness excludes $d$. The next realized history is therefore

\[
H' = I(H,c).
\]

The toy model is simple, but it displays the architecture of the view in one place. Constitutive constraint determines which continuations count as genuine candidates. Selective actualization then distinguishes the merely admissible from the actually eligible. And primeness ensures that actual history proceeds by contextually warranted articulation rather than by arbitrary bundling of whatever could be packaged together.
