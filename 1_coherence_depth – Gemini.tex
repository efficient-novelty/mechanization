...
The Cubical Agda development described in \autoref{sec:mechanization} provides formal counterparts for these results using a split epistemic architecture. The univalent lower bounds (\autoref{thm:adjunction} and \autoref{thm:clutching}) are proven directly as genuine geometric constructions in the Cubical kernel. However, extracting integration latency limits from an embedded compiler trace is intractably complex. Therefore, the trace principle, canonicity constraints, and recurrences (\autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{lem:horn-reduction}, \autoref{lem:telescopic}, \autoref{thm:upper}, \autoref{cor:chrono-window}, \autoref{thm:recurrence}, \autoref{cor:fibonacci}) are mechanized by synthesizing an abstract \emph{interface calculus} parameterized by these geometric bounds. The prose mathematical arguments remain essential, as they provide the type-theoretic justification for the structural axioms of the formalized model.
...
\begin{rem}[Role in the formal model]
In the accompanying Agda formalization (\path{CanonicityDensity.agda}), this metatheoretic derivation is formalized synthetically. The abstract interface calculus defines global admissibility as a record requiring a total action and unique clause constraints, proving that these structural axioms mathematically entail contractibility (and thus maximal density) of the integration choices, rather than computationally extracting this from a deep embedding of syntax.
\end{rem}
...
\begin{rem}[Algebraic trace decomposition]
In the Agda mechanization, this exact coproduct decomposition is the defining property of the structural model's sealed layers: \path{ExplicitSealedLayer} natively stores a sealed layer alongside explicit counting isomorphisms for its payload and trace components, ensuring the interface calculus rigorously tracks these distinct sources of debt without needing to shallowly parse internal syntax trees.
\end{rem}
...
\begin{rem}[Synthetic structural subsumption]
\label{rem:structural-model-upper}
To formalize this stabilization cleanly without a full deep embedding of cubical operational semantics, our Cubical Agda development constructs a synthetic obligation language. In that structural model, marginal higher-arity data are synthetically represented as horn-extension fibers (\path{HornExtensionFiber}), which are structurally defined to be computationally derived. Thus the mechanization establishes exact depth~$2$ for the structural model representing the calculus, rigorously codifying the combinatorial consequences of cubical Kan composition.
\end{rem}
...
\begin{rem}[Genuine native lower bounds]
Unlike the structural upper bounds modeled synthetically by the interface calculus, the explicit binary obstructions below are genuinely deep cubical constructions natively formalized in our Agda repository. They demonstrate directly within the univalent semantics that unary data are insufficient to resolve foundational integration obligations.
\end{rem}
...
\begin{rem}[Recurrence on the structural window]
In the formalization, this theorem is verified within the structural model by computing the combinatorial size of a chronologically counted window, cleanly decoupling the Fibonacci arithmetic from the deep univalent semantics that mathematically justify the window's size.
\end{rem}
...
\subsection{Mechanizing the split architecture}

To formally capture framework evolution without requiring an intractable deep embedding of the compiler's own syntax and operational semantics, the accompanying Cubical Agda development employs a \emph{split epistemic architecture}. It explicitly distinguishes between genuine geometric bounds evaluated natively in the univalent kernel, and an abstract structural interface calculus that models the resulting combinatorial integration debt.

\textbf{Native Geometric Constructions.}
The foundational sources of higher coherence are mechanized as genuine, natively compiled witnesses in Cubical Agda:
\begin{itemize}
    \item \autoref{thm:adjunction} corresponds exactly to \path{Metatheory/AdjunctionBarrier.agda}. It constructs a formal explicit swap obstruction over the two-point type natively using univalence.
    \item \autoref{thm:clutching} corresponds exactly to \path{Geometry/Clutching.agda}. It natively demonstrates that a Hopf clutching datum requires non-contractible binary coherence over the higher-inductive $2$-sphere.
\end{itemize}

\textbf{Structural Interface Calculus.}
The counting combinatorics, upper bounds, and framework metatheory are mechanized by synthesizing an axiomatic structural model:
\begin{itemize}
    \item \textbf{Trace Principle and Canonicity:} In \path{Metatheory/TracePrinciple.agda} and \path{CanonicityDensity.agda}, interfaces and sealed layers are structured algebraically. Totality and uniqueness are formulated as univalent contractibility axioms over a historical interface record, and the exact payload/trace splitting is mechanized via strict counting isomorphisms on an \path{ExplicitSealedLayer} coproduct structure.
    \item \textbf{Upper Bounds:} In \path{Metatheory/KanSubsumption.agda} and \path{Metatheory/UpperBound.agda}, obligations are modeled by a synthetic syntax where \path{StructuralObligation} has a primitive depth-two constructor and extends to deeper history exclusively by attaching a \path{HornExtensionFiber}. Higher obligations are thus derived by construction, structurally guaranteeing the collapse to depth two (\path{Metatheory/ExactDepth.agda} directly equates cubical and structural depth).
    \item \textbf{Recurrence Arithmetic:} In \path{Metatheory/UniversalRecurrence.agda}, the affine latency law evaluates directly on a \path{ChronologicalRecurrenceContext}. The file strictly proves that the size of the supplied recent-layer window algebraically equals the sum of the payload and trace counts, driving the shifted Fibonacci sequence (\path{Core/AffineRecurrence.agda}). The smoke tests in \path{Test/MetatheorySmoke.agda} structurally validate these type bounds within the modeled calculus.
\end{itemize}

By explicitly splitting the codebase this way, the paper avoids overclaiming the scope of its mechanization. The Agda formalization correctly identifies and proves the exact geometric univalent bounds natively, and separately guarantees that \emph{if} a module system behaves according to these bounds, the structural combinatorics rigorously force Fibonacci latency scaling.

\subsection{Theorem map}

The complete paper-to-Agda theorem map across both the native and structural layers is as follows:
...
\item \autoref{thm:trace} corresponds exactly to \path{Metatheory/TracePrinciple.agda} via \path{IntegrationTracePrinciple}, \path{public-counting-normal-form}, and \path{integration-trace-principle}.
...
Because the upper bounds and recurrence limits are structurally evaluated inside the interface calculus, the prose mathematical arguments of \autoref{sec:coherence} remain essential. They provide the deep type-theoretic justification for \emph{why} a univalent foundation correctly satisfies the geometric constraints that fuel the structural recurrence model.
\end{rem}
...
The mechanism behind the recurrence is structural. In the formalized interface calculus, \autoref{thm:canonicity} is realized by \path{global-admissibility-forces-maximal-interface-density}, \autoref{thm:trace} by \path{integration-trace-principle}, and \autoref{cor:chrono-window} by \path{two-layer-chronological-window}. Sealing cost is therefore cleanly identified with the size of the active interface, while resolved coherence data disappear as derivations but remain present in the public signature as trace fields. In the cubical univalent case, the marginal higher-arity data conceptually form horn extensions rather than closed-sphere fillers; in our structural model, this is explicitly embedded so that \path{structural-stabilizes-at-two} acts as an exact upper bound. Concurrently, genuine cubical constructions via \path{adjunction-barrier} and \path{clutching-family-theorem} supply explicit univalent lower bounds outside the structural model. Telescopic subsumption structurally synthesizes older comparisons from adjacent exported traces, yielding a primitive active interface generated by the previous two layers. Under constant payload size, \path{universal-affine-recurrence} specializes to the affine depth-two law, and \path{Delta-bootstrap} drives the shifted Fibonacci sequence.
...
To properly mechanize these findings without intractably attempting to embed an entire module system, the accompanying Cubical Agda formalization operates via a split architecture. It directly verifies the lower bounds natively in Cubical Agda, and it establishes the exact trace and Fibonacci arithmetic within a synthetic interface calculus. \autoref{sec:mechanization} records the exact paper-to-Agda map; representative formal counterparts are \path{cubical-coherence-depth-exactly-two} for the exact depth limit in the structural package, \path{adjunction-barrier} for the native univalent obstruction, \path{cubical-depth-two-law-for-2d-foundations} for the abstract $2$D-foundations wrapper, and \path{Delta-bootstrap} for the shifted Fibonacci corollary. A natural next step is to connect this modeled metatheory more tightly to concrete systems for generating and compiling candidate extensions directly at the syntactic level.
