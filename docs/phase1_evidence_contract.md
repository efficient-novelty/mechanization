...
- `runs/phase1_ci/<run-id>/abinitio_mbtt_structural.csv`
  - Required Phase-4 telemetry columns: `bit_kappa`, `ast_nodes`, `canonical_key`, `decoded_name?`
...
This enforces lane-specific required files, requires `summary.md`, requires acceptance lanes to report zero failures (`Results: ... 0 failed`), requires summary lane entries to include concrete `Results:` lines (not placeholder text), validates required Phase-4 telemetry columns in ab-initio CSV outputs (`bit_kappa`, `ast_nodes`, `canonical_key`, `decoded_name?`), and verifies that main-branch ladder gate emits `ladder-main/ladder_gate.txt` with `pass`.
