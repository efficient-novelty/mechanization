...
- Command (full replay): `cabal run ab-initio -- --strict --mbtt-first --csv <run.csv> [--mbtt-max-candidates N]`
- Command (shadow replay): `cabal run ab-initio -- --strict --phase1-shadow --csv <shadow.csv> [--mbtt-max-candidates N]`
...
### Lane E3 — Strict prefix regression gate to step 7 (required on PR/main)
- Command: `TIMEOUT_S=30 MAX_STEPS=7 REQUIRE_SUCCESS_THROUGH=7 RTS_CORES=-N EXPECTED_NAMES="1:Universe 2:Unit 3:Witness 4:Pi 5:S1 6:Trunc 7:S2" bash engine/scripts/run_prefix_regression.sh runs/phase1_ci/<run-id>/prefix`
- Scope: canonical strict discovery command (`--strict --phase1-shadow --max-steps 7 --prefix-report ... +RTS -N -RTS`) with fixed name assertions for steps 1–7
- Gate: command must exit 0, remain within timeout budget, and emit `prefix/prefix_gate.txt` with first line `pass`
...
- `runs/phase1_ci/<run-id>/abinitio_mbtt_full.csv`
  - Required Phase-4/5 telemetry columns: `bit_kappa`, `ast_nodes`, `canonical_key`, `decoded_name?`, `decode_confidence`, `decode_ambiguity`, `decode_status`
- `runs/phase1_ci/<run-id>/manifest.json`
- `runs/phase1_ci/<run-id>/summary.md` (human-readable lane summary generated by `summarize_phase1_evidence.sh`)
...
- `runs/phase1_ci/<run-id>/prefix/prefix_report.csv`, `prefix/prefix_summary.csv`, `prefix/prefix_gate.txt`, and `prefix/runtime.txt`
...
This enforces lane-specific required files, requires `summary.md`, requires acceptance lanes to report zero failures (`Results: ... 0 failed`), requires summary lane entries to include concrete `Results:` lines (not placeholder text), validates required Phase-4/5 telemetry columns in ab-initio CSV outputs (`bit_kappa`, `ast_nodes`, `canonical_key`, `decoded_name?`, `decode_confidence`, `decode_ambiguity`, `decode_status`), and verifies that main-branch ladder gate emits `ladder-main/ladder_gate.txt` with `pass`.
...
Archived reports and baseline snapshots may still mention retired `--structural` and `--paper` modes.
