...
% ============================================
\begin{abstract}
We introduce the \emph{Principle of Efficient Novelty} (PEN), an information-theoretic model for selecting mathematical frameworks by maximizing novelty $\nu$ per specification effort $\kappa$ under historical integration latency.  Starting from empty intensional type theory, a synthesis engine deterministically orders a 15-step sequence from dependent types and higher inductive spheres to cohesion, connections, curvature, metrics, Hilbert structure, and the Dynamical Cohesive Topos (DCT).

Candidate generation is split: Steps~1--9 are combinatorial, while Steps~10--15 are drawn from a theory-guided HoTT-compatible pool.  Crucially, scoring and selection are mechanical and domain-agnostic: $\nu$ and $\kappa$ are computed from structural AST analysis with no hand-tuned parameters.  The engine rejects mathematically natural alternatives (including arithmetic-first, scheme-theoretic, and measure-theoretic branches) on efficiency grounds.

Four structural results govern the emergence: (i) the Coherence Window Theorem ($d=2$ is required), (ii) Fibonacci integration latency from two-step coherence, (iii) a three-way novelty decomposition (syntactic/topological/logical), and (iv) a fixed-point halt at DCT via internal temporal--tangent closure.  Stress tests over $d \in \{1,2,3\}$ and mechanized verification support uniqueness of the $d=2$ trajectory.  PEN derives the \emph{kinematic framework} of physics, not specific gauge groups, constants, or equations of motion.
\end{abstract}
...
We call this boundary the \emph{G\"odelian Horizon}---the connected-component boundary of the type-theoretic moduli space beyond which only external axiomatic jumps can reach.
...
PEN derives the \emph{kinematic framework} of physics---the type-theoretic and categorical structures that constitute the mathematical language---not specific dynamical laws, gauge groups, coupling constants, or spacetime dimension.
The candidate pool for steps 10--15 is theory-guided (drawn from HoTT-compatible mathematical structures); the engine's contribution is to verify that the selection ordering, and only this ordering, survives the rising bar.
The model assumes \emph{maximal interface density} (each candidate seals against the entire exported interface of the past $d$ layers), a library-quality policy that produces the Fibonacci latency but is not a native requirement of the HoTT type\-checker.
...
\Cref{sec:exponentiality} establishes superlinear novelty growth, the DCT synthesis mechanism, and the G\"odelian Horizon.
...
\Cref{sec:boundaries} addresses four structural objections---algorithmic criticality, the inefficiency of discrete structures, the G\"odelian Horizon, and the DCT novelty count---and shows that each boundary condition is itself informative.
