...
This suggests the entire 15-step arc is an artifact of parameter overfitting---that the MBTT bit-lengths (\cref{def:effort}) were reverse-engineered to force a predetermined outcome.
...
\emph{Second}, the specification cost $\kappa$ is defined via Conditional Kolmogorov Complexity (\cref{def:effort}).
By the Invariance Theorem of Kolmogorov Complexity~\cite{kolmogorov}, changing the prefix-free encoding changes every $\kappa$ value by at most an additive constant $c$ (dependent on the choice of universal Turing machine, independent of the structure being encoded).
Since the selection ratio $\rho = \nu / \kappa$ is evaluated \emph{comparatively} (candidates compete at each step), a uniform additive shift $\kappa \mapsto \kappa + c$ preserves the ordering for all but the tightest margins.

\begin{proposition}[Macroscopic Stability]
\label{prop:macro-stability}
The Genesis Sequence exhibits a \emph{macroscopic attractor} structure: the four evolutionary phases---Bootstrap ($n = 1$--$4$), Geometric Ascent ($n = 5$--$8$), Structural Completion ($n = 9$--$13$), and Synthesis ($n = 14$--$15$)---are robust under any prefix-free re-encoding.
The exact micro-ordering within a phase (e.g., whether $S^2$ precedes or follows PropTrunc) may shuffle under a different encoding, but the macroscopic trajectory
\[
    \text{Dependent Types} \;\to\; \text{Higher Geometry} \;\to\; \text{Modalities} \;\to\; \text{Topos}
\]
is invariant, because the phase boundaries are separated by order-of-magnitude gaps in $\rho$ that no bounded constant can bridge.
\end{proposition}
...
If the algorithmic cost of curvature were even a fraction of a bit higher, the universe would permanently stagnate at flat spacetime---precisely the kind of sharp phase boundary that characterizes critical phenomena.
