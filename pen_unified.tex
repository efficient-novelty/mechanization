...
% ============================================
\begin{abstract}
Is the historical progression of foundational mathematics a product of arbitrary human intuition, or the inevitable trajectory of deterministic logical optimization? We introduce the \emph{Principle of Efficient Novelty} (PEN), a formal, information-theoretic model of mathematical evolution in which a computational agent iteratively extends a formal library by selecting structures that maximize combinatorial derivation power (novelty) relative to algorithmic specification cost (effort).

Operating on an initially empty intensional type theory, PEN autonomously generates the \emph{Genesis Sequence}: a deterministic 15-step trajectory evolving from basic dependent types, through higher inductive spheres and cohesive modalities, culminating in the Dynamical Cohesive Topos of modern physics. We establish that this progression is governed by strict metamathematical laws:

\emph{The Coherence Window Theorem}: Intensional foundations strictly require a historical coherence depth of $d=2$ to natively compute adjoint categorical coherence, whereas extensional foundations require only $d=1$.

\emph{The Complexity Scaling Theorem}: This $d=2$ requirement forces the structural integration costs of new mathematics to scale according to the Fibonacci sequence, allowing sustained exponential growth bounded by the Golden Ratio ($\varphi$).

\emph{The Combinatorial Novelty Theorem}: Generative logical capacity scales superlinearly via schema synthesis, ensuring systemic efficiency permanently outpaces the exponentially rising selection threshold.

\emph{The Spectral Decomposition}: Mathematical evolution intrinsically favors an ``equipartition of logic,'' wherein syntactic (Introduction), topological (Computation), and logical (Elimination) derivation rules contribute to structural survival with approximately equal weight.

Validated by a Haskell synthesis engine and machine-checked Cubical Agda proofs, our results suggest that the core structures of modern geometry and theoretical physics are not empirical conveniences, but the uniquely optimal algorithmic pathways for expanding mathematical logic.
\end{abstract}
...
\begin{table}[H]
\centering
\caption{The Genesis Sequence.  $\nu$ is the Generative Capacity (count of atomic inference rules added to the derivation logic).  Every quantity is computable from the five axioms of \S\ref{sec:framework}; no free parameters have been fitted.}
\label{tab:genesis}
\small
...
The equal-weight property is an emergent structural fact, not a parameter choice.
...
The equal-weight property is the central empirical discovery of this paper.
...
The \emph{spectral mode} computes $\nu$ via its spectral projections $\nu_G + \nu_H + \nu_C$ (\S\ref{sec:decomposition}), using proof-rank clustering for Introduction rules, path-dimension formulas for Computation rules, and capability analysis for Elimination rules.
...
\paragraph{The sample size for the spectral decomposition.}
