...
\]
Using engine-native MBTT traces from \texttt{computeNativeNu}, the measured interaction terms for these three tested routes are
$\Delta \nu_{\mathrm{mech}} = 1, -6, 0$ respectively, yielding strict scores $4.27$, $4.00$, and $5.11$.
All are below the Step~13 bar $5.99$, so the current scalar routes do not yet resolve Step~13 under strict first-use charging.
This is treated as an open synthesis target (find richer interaction schemas or revise the scalar route), not as a bookkeeping exception.
