...
Starting from empty intensional type theory, this rule yields a deterministic 15-step synthesis trajectory: from dependent types and higher-inductive homotopy primitives to cohesion, connections, curvature, metrics, Hilbert functional structure, and finally the Dynamical Cohesive Topos (DCT).  Candidate generation is split between a combinatorial phase (Steps~1--9) and a theory-guided HoTT-compatible phase (Steps~10--15), but scoring is uniform throughout: $\nu$ and $\kappa$ are computed mechanically from structural AST analysis, without hand-tuned parameters or domain-specific rewards.  On this basis the engine rejects several mathematically natural alternatives (e.g., arithmetic-first, scheme-theoretic, and measure-theoretic branches) as less efficient.
