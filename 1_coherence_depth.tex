...
We also call~$d_{\mathrm{obl}}$ the \emph{coherence window} of the foundation. When the paper says simply ``coherence depth,'' it means obligation depth.
\end{defi}
...
This is an elimination statement about $\mu$-minimal normalized public signatures, not a claim that the corresponding exact obligation types are absent from $\Ocal^{(k)}(X)$.
\end{thm}
...
\]
for every $k \geq 3$, and iterating yields $\Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)$ for all $k \geq 2$. Separately, \autoref{lem:horn-reduction} together with \autoref{thm:computational-replacement} yields the surface-syntactic corollary recorded in \autoref{thm:higher-elim}: any arity-$k \geq 3$ trace witness is definitional shadow of lower-arity boundary data and therefore disappears from a $\mu$-minimal public presentation.
...
\midrule
Chosen CCHM-style cubical core / Cubical Agda variation & yes; explicit swap and clutching witnesses & yes; horn reduction, stabilization, and recent-history factorization & yes, with \autoref{thm:computational-replacement} still paper-level & yes \\
