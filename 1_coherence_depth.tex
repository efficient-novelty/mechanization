...
\autoref{thm:higher-elim} & elimination from $\mu$-minimal normalized public signatures & \autoref{lem:horn-reduction}, \autoref{thm:computational-replacement}, and canonical presentation & bridge corollary via \autoref{rem:bridge-target} \\
\autoref{lem:telescopic} & recent-history factorization of surviving binary primitive fields & factorization-complete trace export and the formal chronological-window chain & read through \autoref{rem:bridge-target} \\
...
\autoref{thm:recurrence} & affine recurrence for $\mu$ & \autoref{thm:trace}, \autoref{cor:mu-delta}, and the chronological-window package & formalized abstractly, via \autoref{rem:bridge-target} \\
...
\begin{rem}[Exact scope of the paper-level bridge]
\label{rem:bridge-target}
The unformalized part of \autoref{sec:bridge} is a three-part bridge package, not an additional cubical metatheory:
\begin{enumerate}
  \item raw admissible surface declarations must be connected, via elaboration and sealing, to the explicit canonical trace-presentation interface used in \path{Metatheory/ComputationalReplacement.agda};
  \item presentation-equivalent declarations must be shown to induce canonically isomorphic normalized public signatures and trace sub-signatures;
  \item those canonical normalized signatures must be matched with the abstract counted-interface quantities $P$, $S$, $\Delta$, $\mu$, and $\Ocal^{(k)}$ used in \autoref{sec:framework} and \autoref{sec:coherence}.
\end{enumerate}
In particular, \autoref{thm:computational-replacement} itself is already formalized on canonical trace presentations; the paper-level work is only to connect raw declarations to that interface and then to the counted abstract interface.
\end{rem}
...
Eliminated from a $\mu$-minimal signature & A primitive field may occur in some presentation, but every canonical $\mu$-minimal normalized public signature removes it after passage through presentation equivalence and the bridge package of \autoref{rem:bridge-target}. \\
...
\]
for every $k \geq 3$, and iterating yields $\Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)$ for all $k \geq 2$. Separately, \autoref{lem:horn-reduction}, \autoref{thm:computational-replacement}, and the bridge package of \autoref{rem:bridge-target} yield the surface-syntactic corollary recorded in \autoref{thm:higher-elim}: any arity-$k \geq 3$ trace witness is a definitional shadow of lower-arity boundary data and therefore disappears from a $\mu$-minimal public presentation.
In the taxonomy of \autoref{rem:present-vs-cost}, the higher exact obligation types remain present but contractible at the metatheoretic level, and only passage through \autoref{thm:computational-replacement} and \autoref{rem:bridge-target} turns that fact into elimination from $\mu$-minimal syntax.
\end{proof}
...
\midrule
Chosen CCHM-style cubical core / Cubical Agda realization & yes; explicit swap and clutching witnesses & yes; horn reduction, stabilization, and recent-history factorization & yes; replacement interface formalized, bridge package of \autoref{rem:bridge-target} paper-level & yes \\
...
The formalization is organized around the fixed extension calculus of \autoref{sec:framework}, its horn-computational abstraction layer, its depth theorems, the induced recurrence law, and the clutching construction. \autoref{tab:mechanization-status} first makes the trusted boundary local. Here \emph{fully formalized} means that the paper's statement is realized directly in the cited Agda modules; \emph{formalized abstractly, via \autoref{rem:bridge-target}} means that the Agda development proves the structural theorem and the paper reads it through the three-part bridge package isolated in \autoref{rem:bridge-target}; and \emph{paper-level only} marks that bridge package itself. In particular, \autoref{thm:computational-replacement} is fully formalized on explicit canonical trace presentations; the only unformalized work is connecting raw declarations to that interface and matching the resulting canonical signatures with the counted abstract interface. The itemized map after the table records the exact paper-to-Agda theorem correspondence.
...
\midrule
\autoref{rem:bridge-target} bridge package & paper-level only & none & n/a \\
...
\autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{cor:mu-delta} & formalized abstractly, via \autoref{rem:bridge-target} & \path{Metatheory/CanonicityDensity.agda}, \path{Metatheory/TracePrinciple.agda}, \path{Metatheory/InterfaceCalculus.agda} & yes \\
...
\autoref{thm:horn-interface}, \autoref{cor:cubical-horn-interface} & formalized abstractly, via \autoref{rem:bridge-target} & \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/ChronologicalWindow.agda}, \path{Metatheory/ComputationalReplacement.agda} & yes \\
...
\autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci} & formalized abstractly, via \autoref{rem:bridge-target} & \path{Metatheory/UniversalRecurrence.agda}, \path{Core/DepthOneAffine.agda}, \path{Core/AffineRecurrence.agda} & yes \\
...
\item \autoref{thm:canonicity} is the paper's basis-cardinality reformulation of \path{Metatheory/CanonicityDensity.agda} via \path{CanonicityDensityTheorem} and \path{global-admissibility-forces-maximal-interface-density}, combined with the bridge package of \autoref{rem:bridge-target}.
...
\item \autoref{cor:mu-delta} is the paper's bridge corollary combining \autoref{thm:canonicity}, \autoref{thm:trace}, and the bridge package of \autoref{rem:bridge-target}; the Agda development formalizes the abstract counting theorems, while the passage to the syntactic quantity~$\mu$ is the normalized-signature reading supplied by \autoref{rem:bridge-target}.
...
\item \autoref{thm:computational-replacement} is formalized in \path{Metatheory/ComputationalReplacement.agda}. That module makes the canonical trace-signature interface explicit via \path{CanonicalTraceSignature} and \path{TracePresentation}, packages the presentation theorem itself as \path{computational-replacement-preserves-canonical-presentation}, and exposes the higher-arity elimination consequence via \path{higher-arity-fields-disappear-from-minimal-signature} and \path{higher-arity-computational-replacement}. The only remaining paper-level content there is the bridge package of \autoref{rem:bridge-target}.
\item \autoref{thm:factorization-complete} is the paper's export theorem at the declaration-to-signature boundary isolated in \autoref{rem:bridge-target}: \path{TelescopicSubsumptionView} in \path{Metatheory/KanSubsumption.agda} packages the recent boundary together with the iterated trace chain, while \path{primitive-obligations-factor-through-last-two} in \path{Metatheory/ChronologicalWindow.agda} records that primitive support factors through the last two layers. The paper theorem then reads that formal package through \autoref{rem:bridge-target} as an explicit normalized exported trace basis.
\item \autoref{thm:higher-elim} is the paper's surface-syntactic corollary of \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/TwoDFoundations.agda}, and \path{Metatheory/ComputationalReplacement.agda}: the Agda development formalizes the horn-extension fiber, its canonical inhabitant, stabilization at depth~$2$, the abstract primitive-elimination law, and the canonical-trace replacement interface via \path{HornExtensionFiber}, \path{structural-integration-horn-reduction}, \path{remote-layer-obligation-derived}, \path{structural-obligation-set-equivalence}, \path{structural-stabilizes-at-two}, \path{structural-primitive-eliminates-above-two}, \path{higher-arity-fields-disappear-from-minimal-signature}, and \path{higher-arity-computational-replacement}; the only remaining paper step is the bridge package of \autoref{rem:bridge-target}.
\item \autoref{lem:telescopic} corresponds exactly to \path{Metatheory/KanSubsumption.agda} via \path{TelescopicSubsumptionView}, \path{telescopic-subsumption}, and \path{telescopic-remote-comparison-derived}; its reading as recent-history factorization through normalized public signatures is again the paper's interpretation of that formal trace chain through \autoref{rem:bridge-target}.
\item \autoref{thm:horn-interface} is the paper's stronger horn-computational wrapper. It combines the horn-reduction and contractible-factor package from \path{Metatheory/KanSubsumption.agda} and \path{Metatheory/UpperBound.agda}, the two-layer support factorization theorem \path{primitive-obligations-factor-through-last-two} from \path{Metatheory/ChronologicalWindow.agda}, and the canonical-trace replacement interface of \path{Metatheory/ComputationalReplacement.agda} via \path{CanonicalTraceSignature}, \path{TracePresentation}, \path{higher-arity-fields-disappear-from-minimal-signature}, and \path{higher-arity-computational-replacement}, read through the bridge package of \autoref{rem:bridge-target}. \autoref{cor:cubical-horn-interface} is the paper's checked instantiation of that wrapper for the chosen cubical calculus.
