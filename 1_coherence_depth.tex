...
The Cubical Agda development described in \autoref{sec:mechanization} formalizes the abstract candidate-and-obligation calculus, the cubical upper/lower metatheory, the contractible-factor decomposition of higher exact fibers, the recent-history factorization chain, the canonical-trace replacement interface for $\mu$-minimal elimination, and the abstract recurrence package underlying \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{thm:upper}, \autoref{cor:contractible-factor}, \autoref{cor:chrono-window}, \autoref{thm:horn-interface}, \autoref{cor:cubical-horn-interface}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}. The remaining surface-syntactic readings of \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:factorization-complete}, \autoref{thm:higher-elim}, \autoref{lem:telescopic}, and the $\mu$-based recurrence are then obtained in the paper by combining those formal metatheorems with the bridge results of \autoref{sec:bridge}. \autoref{sec:mechanization} records the exact Agda theorem names and the trusted boundary.
...
\begin{enumerate}
  ...
  \item In the canonical presentation of Property~(3), every historical trace field of arity $k \geq 3$ corresponding to a horn factor of Property~(1) is marked \emph{derived}, and computational replacement by the canonically synthesized horn witness preserves the canonical normalized trace presentation.
...
\[
  \Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)
\]
for every candidate~$X$ and every $k \geq 2$, which is exactly the claimed upper bound on obligation depth.

Property~(5) is already the required two-layer chronological-window statement for primitive support. For the surface-syntactic conclusion, fix an admissible surface declaration~$e$ and any primitive historical field~$\phi$ of arity $k \geq 3$ occurring in one chosen presentation. By Property~(3), its contribution may be read on the canonical normalized trace presentation attached to~$e$. By Property~(4), the corresponding canonical field is marked derived and may be replaced by the horn-computed witness without changing the canonical normalized trace presentation. Hence $\phi$ contributes no irreducible primitive trace schema in any $\mu$-minimal representative. Repeating this finite replacement for each arity-$k \geq 3$ field yields a presentation-equivalent declaration whose normalized trace signature has no irreducible historical field above arity~$2$. This is exactly the asserted elimination statement, and by \autoref{def:depth} it gives $d_\mu \leq 2$.
\end{proof}
...
\autoref{thm:horn-interface}, \autoref{cor:cubical-horn-interface} & formalized abstractly, read through bridge & \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/ChronologicalWindow.agda}, \path{Metatheory/ComputationalReplacement.agda} & yes \\
...
\item \autoref{thm:horn-interface} is the paper's stronger horn-computational wrapper. It combines the horn-reduction and contractible-factor package from \path{Metatheory/KanSubsumption.agda} and \path{Metatheory/UpperBound.agda}, the two-layer support factorization theorem \path{primitive-obligations-factor-through-last-two} from \path{Metatheory/ChronologicalWindow.agda}, and the canonical-trace replacement interface of \path{Metatheory/ComputationalReplacement.agda} via \path{CanonicalTraceSignature}, \path{TracePresentation}, \path{higher-arity-fields-disappear-from-minimal-signature}, and \path{higher-arity-computational-replacement}, read through the bridge-level normalization package of \autoref{sec:bridge}. \autoref{cor:cubical-horn-interface} is the paper's checked instantiation of that wrapper for the chosen cubical calculus.
