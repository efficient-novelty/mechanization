...
\begin{abstract}
We study framework extension over a fixed cubical object theory rather than over ``cubical type theory'' in the abstract. The theorem target is a cubical extension calculus built on a chosen CCHM-style cubical core with univalence and Kan composition, together with a discipline of globally acting opaque extensions whose public interfaces are counted only up to canonical telescope isomorphism. We call the least historical depth at which the resulting normalized obligation types stabilize up to canonical equivalence the coherence depth. Within this calculus we prove a payload-aware trace principle decomposing each sealed public interface into new core payload and resolved coherence trace. The cubical metatheory then yields a depth-two theorem: the higher-arity tail is subsumed by cubical horn filling, this stabilization is realized by a genuine two-layer chronological window, and a binary obstruction remains genuinely independent. Abstracting that proof pattern yields a universal affine recurrence for integration latency. In extensional systems with UIP the depth collapses to~$1$. In the fixed fully coupled cubical extension calculus studied here the depth is~$2$, so
\[
  \Delta_{n+1} = \Delta_n + \Delta_{n-1} + 2c
\]
under constant payload size~$c$; after the shift $U_n=\Delta_n+2c$, the sequence is Fibonacci. The accompanying Cubical Agda development formalizes the extension-calculus layer together with the cubical witness theorems used in the depth argument; \autoref{sec:mechanization} records the exact paper-to-Agda map and the trusted boundary.
\end{abstract}
...
\begin{enumerate}
\item We fix a cubical extension calculus over a chosen CCHM-style core, formalize its states, surface extensions, elaboration, sealed extensions, promoted interface packages, active interfaces, and integration latency, and distinguish orthogonal extensions from globally acting foundational core extensions. The payload and latency counts are defined on flattened telescope normal forms, so they are invariant under admissible interface refactorings such as rebundling records or splitting one interface into several shells. The bridge section then proves that elaboration, sealing, and counting normalization turn explicit surface declarations into canonical normalized public signatures, so the abstract counts are theorems about extension syntax over the fixed cubical core. Within this framework we prove a basis-sensitive coverage/minimality theorem for the fully coupled foundational-core regime and a payload-aware \emph{Integration Trace Principle}, decomposing the canonical basis of a sealed public interface into its new public payload and the resolved obligations discharged in sealing it.
    \item We define coherence depth for this extension calculus and prove the sharp cubical/extensional dichotomy. In extensional type theory with UIP, the depth is~$1$. In the fixed fully coupled cubical extension calculus, the depth is~$2$. The depth-two statement is supported by three convergent arguments: a dimensional upper bound from weak $\omega$-groupoid semantics and cubical Kan filling, stated as an exact obligation-set equivalence because the marginal higher-arity data are normalized as horn extensions rather than closed-sphere fillers; a lower bound from an explicit swap-induced binary sealing obstruction, with adjunction coherence as a categorical reading of the same binary phenomenon; and a topological lower-bound family supplied by clutching constructions for principal bundles over spheres. The passage from the dimensional bound to a genuine two-step time window is made explicit by a telescopic subsumption lemma using cubical Kan filling.
    \item We combine the trace principle with the coherence-depth theorem to obtain a universal affine recurrence for integration latency. Under constant payload size~$c$, the depth-two cubical case specializes to
    \[
      \Delta_{n+1} = \Delta_n + \Delta_{n-1} + 2c,
    \]
    and the shifted sequence $U_n=\Delta_n+2c$ is Fibonacci. The associated structural inflation factor still converges to the Golden Ratio.
\end{enumerate}

The Cubical Agda development described in \autoref{sec:mechanization} gives exact formal counterparts for \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{lem:telescopic}, \autoref{thm:upper}, \autoref{cor:chrono-window}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}; \autoref{sec:mechanization} records the exact Agda theorem names and the trusted boundary. The dimensional, categorical, and topological arguments remain part of the presentation because they expose the theorem architecture and its examples, while the extension-calculus layer and the main depth claims are also formalized.

\subsection{Related work}
\label{sec:related}

The higher-dimensional background comes from the interpretation of intensional identity types as weak $\omega$-groupoids, developed by Lumsdaine~\cite{lumsdaine} and van den Berg and Garner~\cite{vdberg-garner}. Several cubical calculi and implementations appear in the literature; the operational setting fixed in this paper is the CCHM-style cubical core of Cohen, Coquand, Huber, and M\"ortberg~\cite{cubical}, together with higher inductive types and Kan composition~\cite{cchm-hits}. Cubical Agda~\cite{cubical-agda} provides the mechanization environment and the native cubical programs used in the development, but the theorem target of the paper is the extension calculus defined below, not the full Agda surface language. For comparison with mixed weak/strict equality foundations, see the two-level presentations of Altenkirch, Capriotti, and Kraus and of Annenkov, Capriotti, Kraus, and Sattler~\cite{altenkirch-strict,annenkov-2ltt}.
...
\[
  e \approx e',
\]
when their elaborated sealed extensions determine the same public typing judgment up to definitional equality, canonical telescope isomorphism, and transparent internal normalization. The \emph{minimal opaque cost} of~$e$ is
\[
  \mu(e) := \min \bigl\{\, |N(e')| \;\big|\; e' \approx e \,\bigr\}.
\]
\end{defi}
...
If $e \approx e'$, then $N(e)$ and $N(e')$ are canonically isomorphic. Consequently every presentation-equivalence class admits a canonical normalized public signature up to isomorphism, and
\[
  \mu(e) = |N(e)|.
\]
\end{prop}
...
In the fixed fully coupled cubical extension calculus, every sealing step at stage~$n+1$ has a genuine chronological window of size~$2$: all primitive obligations are generated by the exported traces of $L_n$ and $L_{n-1}$, while comparisons with layers $L_{n-r}$ for $r \geq 2$ are algorithmically synthesized by iterated Kan filling. Equivalently, the pair $(L_n,L_{n-1})$ forms a chronological Markov blanket for subsequent sealing.
\end{cor}
...
\begin{proof}
By \autoref{thm:canonicity}, the next sealing cost is the size of the active basis interface:
\[
  \Delta_{n+1} = |I_{n,\mathrm{bas}}^{(d)}|.
\]
...
\[
  |S_{\mathrm{bas}}(L_{n-j})| = \Delta_{n-j} + \kappa_{n-j}
\]
...
The formalization includes both direct cubical metatheory and a distilled extension calculus. The UIP collapse, horn-extension computation, telescopic subsumption, lower-bound obstruction, exact depth-two corollary, and clutching-family witness are mechanized against cubical or UIP primitives themselves. The canonicity, trace, refactoring, recurrence, and abstract $2$D-foundations layers then package those results as the sealed-interface and counting calculus studied in the paper. In the current paper presentation, the basis-sensitive minimality reading of the canonicity theorem is obtained by combining that formal density layer with the bridge normalization results of \autoref{sec:bridge}. The recurrence layer is therefore distilled, but it rests on explicit metatheoretic theorems rather than on an interface sketch alone.
