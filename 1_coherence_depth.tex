...
\]
In the mechanization, the admissible presentation moves are generated by the explicit \path{PresentationStep} constructors, including reassociation, record splitting/bundling, currying/uncurrying, transport of fields, transparent alias insertion/deletion, and duplicate-derived deletion.
\end{defi}
...
\begin{enumerate}
  \item raw admissible surface declarations are connected, via elaboration and sealing, to canonical normalized signatures by \path{Metatheory/RawStructuralSyntax.agda}, \path{Metatheory/RawStructuralTyping.agda}, and \path{Metatheory/SurfaceNormalizationBridge.agda};
  \item presentation-equivalent canonical trace normal forms preserve support, primitive cost, and~$\mu$ by the explicit \path{PresentationStep} generators in \path{Metatheory/PresentationEquivalence.agda} and \path{Metatheory/MuInvariance.agda};
  \item the resulting canonical normalized signatures are matched with the counted-interface quantities by \path{normalized-signature-matches-counted-interface}, and well-typed raw structural clauses are identified with the horn-generated trace image by \path{Metatheory/SurfaceToHornImage.agda}.
\end{enumerate}
...
\midrule
\autoref{rem:bridge-target} fixed raw bridge package & fully formalized for the fixed raw extension calculus & \path{Metatheory/RawStructuralSyntax.agda}, \path{Metatheory/RawStructuralTyping.agda}, \path{Metatheory/SurfaceNormalizationBridge.agda}, \path{Metatheory/SurfaceToHornImage.agda}, \path{Metatheory/CanonicalTelescope.agda}, \path{Metatheory/TraceCostNormalForm.agda}, \path{Metatheory/PresentationEquivalence.agda}, \path{Metatheory/MuInvariance.agda} & n/a \\
\autoref{thm:computational-replacement} & fully formalized on explicit canonical trace presentations & \path{Metatheory/ComputationalReplacement.agda}, \path{Metatheory/MuInvariance.agda} & yes \\
\autoref{prop:transparent} & fully formalized & \path{Metatheory/InterfaceCalculus.agda} & no \\
\autoref{thm:basis-choice} & paper-level explanatory; counted active-basis counterpart formalized & \path{Metatheory/FiniteInterfaceBasis.agda}, \path{Metatheory/ActiveBasisContract.agda} & counted interface \\
\autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{cor:mu-delta} & formalized through the fixed raw bridge & \path{Metatheory/CanonicityDensity.agda}, \path{Metatheory/TracePrinciple.agda}, \path{Metatheory/InterfaceCalculus.agda}, \path{Metatheory/SurfaceNormalizationBridge.agda} & yes \\
...
\autoref{thm:factorization-complete}, \autoref{thm:higher-elim}, \autoref{lem:telescopic} & formalized through the fixed raw bridge, with replacement formalized & \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/ChronologicalWindow.agda}, \path{Metatheory/ComputationalReplacement.agda}, \path{Metatheory/SurfaceToHornImage.agda} & yes \\
\autoref{thm:horn-interface}, \autoref{cor:cubical-horn-interface} & formalized through the fixed raw bridge & \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/ChronologicalWindow.agda}, \path{Metatheory/ComputationalReplacement.agda}, \path{Metatheory/SurfaceToHornImage.agda} & yes \\
...
\autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci} & formalized; Fibonacci is the full-coupling endpoint of sparse recurrence & \path{Metatheory/UniversalRecurrence.agda}, \path{Metatheory/SparseDependencyRecurrence.agda}, \path{Metatheory/FullCouplingEnvelope.agda}, \path{Core/DepthOneAffine.agda}, \path{Core/AffineRecurrence.agda} & yes \\
Case-study audit data & fully formalized and audited & \path{CaseStudies/UniverseExtension.agda}, \path{CaseStudies/GlobalModality.agda}, \path{CaseStudies/PromotedInterface.agda}, \path{CaseStudies/SparseDatatype.agda}, \path{scripts/coherence_depth_audit.py} & no \\
...
\item \autoref{rem:bridge-target} is mechanized for the fixed raw extension calculus. The syntax and admissibility boundary are in \path{Metatheory/RawStructuralSyntax.agda} and \path{Metatheory/RawStructuralTyping.agda}; normalization to canonical payload and trace signatures is in \path{Metatheory/SurfaceNormalizationBridge.agda} via \path{normalizeRawExtension}, \path{raw-extension-elaborates-to-candidate}, \path{raw-extension-normalizes-to-canonical-signature}, \path{normalization-respects-presentation-equivalence}, and \path{normalized-signature-matches-counted-interface}; and the raw-to-horn image theorem is in \path{Metatheory/SurfaceToHornImage.agda} via \path{surface-to-horn-normal-form}, \path{higher-structural-fields-derived}, \path{horn-image-complete-for-structural-clauses}, and \path{raw-structural-normalizes-to-horn}.
\item \autoref{prop:canonical-presentation} and the $\mu$-invariance part of \autoref{def:mu} are mechanized by \path{Metatheory/CanonicalTelescope.agda}, \path{Metatheory/TraceCostNormalForm.agda}, \path{Metatheory/PresentationEquivalence.agda}, and \path{Metatheory/MuInvariance.agda}. The explicit presentation generators are the \path{PresentationStep} constructors, and \path{mu-invariant-under-presentation-equivalence} is the corresponding \path{mu} invariance theorem.
\item \autoref{thm:basis-choice} is a paper-level subset-family reading of the counted active-interface package. Its checked counterpart takes the counted active interface as primitive and formalizes canonical finite basis families and their cardinality/action equivalence in \path{Metatheory/FiniteInterfaceBasis.agda} via \path{basis-families-exist}, \path{basis-family-cardinality-invariant}, and \path{basis-action-equivalence}.
\item \autoref{thm:canonicity} is the paper's basis-cardinality reformulation of \path{Metatheory/CanonicityDensity.agda} via \path{CanonicityDensityTheorem} and \path{global-admissibility-forces-maximal-interface-density}, combined with the fixed raw bridge of \autoref{rem:bridge-target}.
...
\item \autoref{thm:computational-replacement} is formalized in \path{Metatheory/ComputationalReplacement.agda}. That module makes the canonical trace-signature interface explicit via \path{CanonicalTraceSignature} and \path{TracePresentation}, packages the presentation theorem itself as \path{computational-replacement-preserves-canonical-presentation}, and exposes the higher-arity elimination consequence via \path{higher-arity-fields-disappear-from-minimal-signature} and \path{higher-arity-computational-replacement}. \path{Metatheory/MuInvariance.agda} connects this to \path{mu} via \path{computational-replacement-preserves-mu}.
\item \autoref{thm:factorization-complete} is the export theorem at the declaration-to-signature boundary isolated in \autoref{rem:bridge-target}: \path{TelescopicSubsumptionView} in \path{Metatheory/KanSubsumption.agda} packages the recent boundary together with the iterated trace chain, \path{primitive-obligations-factor-through-last-two} in \path{Metatheory/ChronologicalWindow.agda} records that primitive support factors through the last two layers, and \path{SurfaceToHornImage.agda} records that admissible raw structural clauses normalize to the horn image.
\item \autoref{thm:higher-elim} is the surface-syntactic corollary of \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/TwoDFoundations.agda}, \path{Metatheory/ComputationalReplacement.agda}, and \path{Metatheory/SurfaceToHornImage.agda}: the Agda development formalizes the horn-extension fiber, its canonical inhabitant, stabilization at depth~$2$, the abstract primitive-elimination law, the canonical-trace replacement interface, and the raw structural horn image via \path{HornExtensionFiber}, \path{structural-integration-horn-reduction}, \path{remote-layer-obligation-derived}, \path{structural-obligation-set-equivalence}, \path{structural-stabilizes-at-two}, \path{higher-arity-fields-disappear-from-minimal-signature}, \path{higher-arity-computational-replacement}, \path{higher-structural-fields-derived}, and \path{higher-raw-structural-traces-derived}.
\item \autoref{lem:telescopic} corresponds to \path{Metatheory/KanSubsumption.agda} via \path{TelescopicSubsumptionView}, \path{telescopic-subsumption}, and \path{telescopic-remote-comparison-derived}; its reading as recent-history factorization through normalized public signatures is connected to admissible raw structural traces by \path{SurfaceToHornImage.agda}.
\item \autoref{thm:horn-interface} is the paper's stronger horn-computational wrapper. It combines the horn-reduction and contractible-factor package from \path{Metatheory/KanSubsumption.agda} and \path{Metatheory/UpperBound.agda}, the two-layer support factorization theorem \path{primitive-obligations-factor-through-last-two} from \path{Metatheory/ChronologicalWindow.agda}, the canonical-trace replacement interface of \path{Metatheory/ComputationalReplacement.agda}, and the fixed raw bridge of \autoref{rem:bridge-target}. \autoref{cor:cubical-horn-interface} is the checked instantiation of that wrapper for the chosen cubical calculus.
...
\item \autoref{thm:recurrence} corresponds to \path{Metatheory/UniversalRecurrence.agda} via \path{WindowedRecurrenceContext}, \path{ChronologicalRecurrenceContext}, \path{UniversalAffineRecurrence}, \path{universal-affine-recurrence}, and \path{universal-affine-recurrence-from-coherence}, read through \autoref{cor:mu-delta} so that the paper's surface-syntactic recurrence is stated over~$\mu$. The sparse dependency refinement is formalized in \path{Metatheory/SparseDependencyRecurrence.agda} via \path{sparse-windowed-recurrence}, \path{transparent-growth-zero-footprint}, and \path{orthogonal-extension-below-full-envelope}.
...
\item \autoref{cor:fibonacci} corresponds to the fully coupled endpoint of the sparse dependency calculus. The endpoint itself is formalized in \path{Metatheory/FullCouplingEnvelope.agda} via \path{full-coupling-envelope}, \path{full-coupling-specializes-sparse-recurrence}, and \path{full-coupling-depth-two-affine-law}, while the arithmetic specialization is in \path{Core/AffineRecurrence.agda} via \path{Delta-bootstrap}, \path{U-bootstrap-closed}, and \path{tau-bootstrap-closed}; the code's \path{Delta}-named arithmetic sequence is the paper's $\mu$-sequence after the bridge identification of \autoref{cor:mu-delta}.
...
\item The case-study audit is checked by \path{CaseStudies/UniverseExtension.agda}, \path{CaseStudies/GlobalModality.agda}, \path{CaseStudies/PromotedInterface.agda}, \path{CaseStudies/SparseDatatype.agda}, the YAML fixtures in \path{runs/coherence_depth_case_studies/}, and \path{scripts/coherence_depth_audit.py}. These cover full-coupling universe/global examples, a promoted active-basis interface, a transparent zero-footprint lemma extension, a sparse datatype, and a refactored universe presentation with the same~$\mu$.
\end{itemize}
}
...
\path{Metatheory/UniversalRecurrence.agda} proves \path{universal-affine-recurrence}; \path{Metatheory/SparseDependencyRecurrence.agda} packages sparse dependency footprints; \path{Metatheory/FullCouplingEnvelope.agda} identifies the fully coupled endpoint; and \path{Core/DepthOneAffine.agda} and \path{Core/AffineRecurrence.agda} package the exact arithmetic specializations \path{depth1-affine-growth}, \path{Delta-depth1-closed}, \path{tau-depth1-closed}, \path{Delta-bootstrap}, \path{U-bootstrap-closed}, and \path{tau-bootstrap-closed}. In particular, the full-coupling endpoint specializes the payload-free counting law to the payload-aware recurrence
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + 2c,
\]
...
The formalization establishes the cubical exact upper- and lower-bound metatheory, the abstract candidate-and-obligation calculus, the canonical-trace replacement interface, the fixed raw extension-calculus bridge, and the recurrence packages built on them. The exact paper-to-Agda correspondences are listed above; the cited modules use no \texttt{postulate}; and the clutching module is checked separately only because its HIT imports live on the Cubical library's standard natural-number interface rather than on \path{Core/Nat.agda}. The mathematical arguments of \autoref{sec:coherence} remain important as explanation and as links to the categorical and topological examples. The bridge scope is exactly the fixed raw calculus of \autoref{rem:bridge-target}; it is not a parser or elaboration theorem for arbitrary Cubical Agda programs.
\end{rem}
...
The mechanism behind the recurrence is structural. In the formalization, \autoref{thm:canonicity} is read through \path{global-admissibility-forces-maximal-interface-density} together with the fixed raw bridge of \autoref{rem:bridge-target}, \autoref{thm:trace} by \path{integration-trace-principle}, and \autoref{cor:chrono-window} by \path{two-layer-chronological-window}. Sealing cost is therefore identified with the size of the active basis interface, while resolved coherence data disappear as derivations but remain present in the public signature as trace fields. In the cubical case, the marginal higher-arity data are horn extensions rather than closed-sphere fillers; \path{structural-stabilizes-at-two} and \path{cubical-coherence-depth-exactly-two} make the exact stabilization statement precise, the replacement interface of \path{Metatheory/ComputationalReplacement.agda} and the \path{mu} invariance theorem of \path{Metatheory/MuInvariance.agda} turn cubically synthesized higher witnesses into elimination from the $\mu$-minimal part of a canonical trace presentation, and \path{primitive-obligations-factor-through-last-two} gives the recent-history factorization underlying the two-layer window. The explicit swap obstruction and the clutching family supply exact lower-bound witnesses. Under constant payload size the full-coupling endpoint \path{full-coupling-depth-two-affine-law} specializes the sparse recurrence to the affine depth-two law, and \path{Delta-bootstrap} gives the shifted Fibonacci corollary.
