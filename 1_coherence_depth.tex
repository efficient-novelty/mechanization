...
\begin{thm}[Elaboration to sealed candidates]
...
\begin{enumerate}
  ...
  \]
  \item a sealed layer $L(e)$ exported through an opaque public interface,
  \item and a normalized public signature $N(e)$ obtained from that exported interface by counting normalization.
\end{enumerate}
...
\begin{proof}
Point~(1) is the surface-language form of the decomposition proved abstractly in \autoref{thm:trace}: after elaboration and sealing, every visible field is either newly introduced payload or the public trace of a resolved interaction with prior interface data. Point~(2) follows because counting normalization removes only presentation artifacts: reassociation of telescopes, transparently derivable fields, and other data uniformly synthesized by definitional equality or the generic structure of the ambient cubical calculus. Thus every irreducible field that remains visible after sealing survives into $N(e)$, and no extra irreducible field is created by normalization. Point~(3) is then immediate from the definitions of payload size, public schema set, integration latency, and induced obligation sets in \autoref{sec:framework} and \autoref{sec:coherence}: those quantities are computed exactly from the normalized public signature and its induced obligations.
\end{proof}

\begin{prop}[Canonical presentations]
\label{prop:canonical-presentation}
If $e \approx e'$, then $N(e)$ and $N(e')$ are canonically isomorphic. Consequently the minimal opaque cost~$\mu(e)$ is well-defined and invariant under admissible refactoring of extension declarations.
\end{prop}

\begin{proof}
Presentation equivalence identifies elaborated sealed extensions whenever they induce the same public typing judgment up to definitional equality, canonical telescope isomorphism, and transparent internal normalization. The counting normal form forgets exactly those presentation differences and nothing more. Therefore $N(e)$ and $N(e')$ are canonically isomorphic, and the cardinality of a minimal normalized opaque presentation depends only on the presentation-equivalence class of~$e$.
\end{proof}

\begin{rem}
\label{rem:bridge-to-counting}
Later sections continue to write $\Delta_k$ for the sealing cost of the chosen normalized presentation exported at stage~$k$. The bridge theorems above are what let those counts be read as counts of explicit extension declarations over the fixed cubical core, rather than as a freestanding bookkeeping model.
\end{rem}
