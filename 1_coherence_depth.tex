...
The present version is a first full draft. The Cubical Agda development described in \autoref{sec:mechanization} now gives exact theorem-facing counterparts for \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{lem:telescopic}, \autoref{thm:upper}, \autoref{cor:chrono-window}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}; that section records the exact Agda theorem names and trust boundary. The dimensional, categorical, and topological arguments are still written here in condensed proof form because they explain the theorem architecture and its examples, but the paper's main depth claims no longer rest on prose alone.
...
For the purposes of the present paper, the relevant Agda artifact splits into an integrated theorem-facing package exported from \path{PEN.agda}, a standalone geometric HIT module \path{Geometry/Clutching.agda}, and the regression imports \path{Test/MetatheorySmoke.agda} and \path{Test/ClutchingSmoke.agda}. The point of this section is therefore to record the exact paper-to-Agda theorem map rather than merely to list computational ingredients.
...
\item \autoref{thm:trace} corresponds exactly to \path{Metatheory/TracePrinciple.agda} via \path{IntegrationTracePrinciple}, \path{public-counting-normal-form}, and \path{integration-trace-principle}, built on the explicit sealed-layer surface \path{ExplicitSealedLayer} from \path{Metatheory/InterfaceCalculus.agda}.
...
\item \autoref{thm:clutching} corresponds exactly to the standalone \path{Geometry/Clutching.agda} via \path{clutching-family-theorem}, \path{hopf-binary-clutching-nontrivial}, and \path{clutching-horn-extension-fiber-contractible}.
...
No theorem-facing module in \path{Metatheory/}, \path{Core/}, \path{PEN.agda}, or the standalone \path{Geometry/Clutching.agda} uses \texttt{postulate}. Postulates do appear elsewhere in the repository, notably in exploratory \path{Experiments/} files and generated \path{bridge/} stubs, but those modules are outside the theorem map above and are not part of the present paper's trust claim.

The trusted base is Agda with Cubical support together with the Cubical library declared in \path{pen.agda-lib}. The integrated theorem package relies on standard Cubical foundations modules plus the repository-local arithmetic surface \path{Core/Nat.agda}; the standalone clutching module additionally trusts the Cubical library's standard higher-inductive circle and suspension modules \path{Cubical.HITs.S1.Base} and \path{Cubical.HITs.Susp.Base}, together with the standard equivalence, isomorphism, and univalence infrastructure.

The artifact formalizes both actual cubical metatheory and a distilled interface calculus. The UIP collapse, horn-extension computation, telescopic subsumption, lower-bound obstruction, exact depth-two corollary, and clutching-family witness are mechanized against the actual cubical/UIP metatheoretic primitives. The canonicity, trace, refactoring, recurrence, and abstract $2$D-foundations layers then package those results as a theorem-facing sealed-interface and counting calculus. Thus the recurrence-facing surface is distilled, but the cubical depth theorem it rests on is not merely an interface sketch.
...
\path{Metatheory/UniversalRecurrence.agda} proves the paper-facing theorem \path{universal-affine-recurrence}, while \path{Core/DepthOneAffine.agda} and \path{Core/AffineRecurrence.agda} package the exact arithmetic specializations \path{depth1-affine-growth}, \path{Delta-depth1-closed}, \path{tau-depth1-closed}, \path{Delta-bootstrap}, \path{U-bootstrap-closed}, and \path{tau-bootstrap-closed}. In particular, \path{Core/AffineRecurrence.agda} strengthens the old normalized counting story to the payload-aware law
\[
  \Delta_{n+1} = \Delta_n + \Delta_{n-1} + 2c,
\]
...
The present Agda artifact formalizes both the foundational causes and the combinatorial consequences of the coherence window at theorem level. The exact paper-to-Agda correspondences are listed above; the theorem-facing modules involved in that map use no \texttt{postulate}; and the standalone clutching module is checked separately only because its HIT imports live on the Cubical library's standard natural-number interface rather than on the repository-local \path{Core/Nat.agda}. The mathematical arguments of \autoref{sec:coherence} remain important as explanation and as links to the categorical and topological examples, but they no longer stand in for an absent theorem-level mechanization of the paper's main claims.
\end{rem}
...
The current Cubical Agda artifact therefore matches the paper at theorem level rather than only at ingredient level. \autoref{sec:mechanization} records the exact paper-to-Agda map; representative theorem-facing counterparts are \path{history-truncates-to-one} for the UIP collapse, \path{cubical-coherence-depth-exactly-two} and \path{cubical-chronological-window-size-exactly-two} for the cubical depth package, \path{cubical-depth-two-law-for-2d-foundations} for the abstract $2$D-foundations wrapper, and \path{Delta-bootstrap} for the shifted Fibonacci corollary. As that section makes explicit, the trust boundary is the Agda/Cubical kernel, the declared Cubical library, and the repository-local arithmetic surface, with no theorem-facing postulates; what is abstracted into interface calculus is the sealed-layer/counting presentation, not the cubical depth core. The next natural step is therefore not to formalize the depth theorem package itself, but to connect this metatheory layer more tightly to the repository's oracle and candidate-generation components.
