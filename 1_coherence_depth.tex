...
\item We fix a cubical extension calculus over a chosen CCHM-style core, formalize its states, surface extensions, elaboration, sealed extensions, promoted interface packages, active interfaces, and integration latency, and distinguish orthogonal extensions from globally acting foundational core extensions. The payload and latency counts are defined on flattened telescope normal forms, so they are invariant under admissible interface refactorings such as rebundling records or splitting one interface into several shells. The bridge section then proves that elaboration, sealing, and counting normalization turn explicit surface declarations into canonical normalized public signatures, so the abstract counts are theorems about extension syntax over the fixed cubical core. Within this framework we prove a basis-sensitive coverage/minimality theorem for the fully coupled foundational-core regime, a payload-aware \emph{Integration Trace Principle}, and a presentation-invariant minimal opaque cost~$\mu$ measuring the irreducible trace fields that survive in canonical public form.
    \item We define coherence depth for this extension calculus and prove the sharp cubical/extensional dichotomy. In extensional type theory with UIP, the depth is~$1$. In the fixed fully coupled cubical extension calculus, the depth is~$2$. The depth-two statement is supported by four complementary ingredients: an exact upper bound on normalized obligation sets showing $\Ocal^{(k)}(X)\simeq\Ocal^{(2)}(X)$ for $k\geq 2$; a separate surface-syntactic corollary showing that primitive historical fields of arity at least~$3$ are eliminated from $\mu$-minimal public signatures by cubical Kan operations; a recent-history factorization theorem showing that every surviving binary clause factors through the previous two normalized signatures; and lower bounds from an explicit swap-induced binary sealing obstruction, read categorically by adjunction coherence, together with a topological exact family supplied by clutching constructions for principal bundles over spheres.
    \item We combine the trace principle with the coherence-depth theorem to obtain a universal affine recurrence for minimal opaque cost. Under constant payload size~$c$, the depth-two cubical case specializes to
    \[
      \mu_{n+1} = \mu_n + \mu_{n-1} + 2c,
    \]
...
Three notions must be kept distinct. An obligation may be absent; it may be present but contractible; or it may be present while still being eliminable from every $\mu$-minimal public presentation. \autoref{def:depth} is a statement about exact normalized obligation types. The surface-syntactic quantity~$\mu$ asks a different question, namely which trace fields survive in canonical normalized public form. Accordingly, ``derivable'', ``contractible'', ``eliminable from a minimal presentation'', and ``not present'' are never used interchangeably in this paper.
\end{rem}
...
\label{def:depth}
A foundation, equivalently a fixed extension calculus together with its sealing discipline, has \emph{coherence depth}~$d$ if $d$ is the least integer such that for every candidate~$X$ and all $k \geq d$ there is a canonical equivalence
\[
...
The point of \autoref{def:depth} is that beyond depth~$d$, the normalized obligation types themselves stop changing up to canonical equivalence. This is stronger than saying merely that deeper history contributes no new minimal opaque cost. Once that exact stabilization has been upgraded to a genuine chronological window, the size of the primitive active basis interface controls the recurrence law for integration debt.
...
From now on we specialize the paper's fixed theorem object to the fully coupled cubical extension calculus built over the chosen CCHM-style core~\cite{cubical,cchm-hits}, with univalence and Kan composition available in the ambient operational semantics. The next ingredients are deliberately complementary: exact obligation-set stabilization gives the upper bound on coherence depth, higher-coherence elimination gives the separate syntactic upper bound on $\mu$-minimal public presentations, recent-history factorization turns that depth bound into a genuine two-step time window, and the explicit swap obstruction together with the clutching family give lower-bound witnesses. Together they pin down exact depth~$2$.
...
\]
Then there exists a globally acting promoted interface candidate whose unary clauses do not determine its sealing: sealing induces a genuine binary obligation over~$p_{\mathsf{swap}}$. Consequently the coherence depth is at least~$2$.
\end{thm}
...
\begin{cor}
\label{cor:d2}
In the fixed fully coupled cubical extension calculus considered above, the coherence depth is exactly~$2$.
\end{cor}
...
For every foundation whose coherence depth~$d$ is realized by a chronological window of size~$d$, the minimal opaque cost satisfies
\[
  \mu_{n+1} = \sum_{j=0}^{d-1} \bigl(\mu_{n-j} + \kappa_{n-j}\bigr)
  \qquad
  (n \geq d).
\]
\end{thm}
...
\label{cor:fibonacci}
In the fixed fully coupled cubical extension calculus, if the coherence depth is~$2$ and every sealed layer contributes core payload size~$c$, then
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + 2c.
\]
If we define
...
\end{enumerate}
Then the foundation has coherence depth~$2$. Under the uniform payload hypothesis $\kappa_n=c$, its minimal opaque cost satisfies
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + 2c,
\]
