...
The Cubical Agda development described in \autoref{sec:mechanization} formalizes the abstract candidate-and-obligation calculus, the cubical upper/lower metatheory, the recent-history factorization chain, and the abstract recurrence package underlying \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{thm:upper}, \autoref{cor:chrono-window}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}. The surface-syntactic readings of \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:higher-elim}, \autoref{lem:telescopic}, and the $\mu$-based recurrence are then obtained in the paper by combining those formal metatheorems with the bridge results of \autoref{sec:bridge}. \autoref{sec:mechanization} records the exact Agda theorem names and the trusted boundary.
...
\begin{defi}[Chosen basis of the historical interface]
\label{def:basis-sites}
For each sealed layer $L$, fix a finite family
\[
  S_{\mathrm{bas}}(L) \subseteq S(L)
\]
of \emph{basis schemas} with the following properties:
\begin{enumerate}
...
\end{enumerate}
For $k \geq 1$, the \emph{historical basis of depth~$k$} at stage~$n$ is
\[
  I_{n,\mathrm{bas}}^{(k)} := \biguplus_{j=0}^{k-1} S_{\mathrm{bas}}(L_{n-j}).
\]
Its elements are the \emph{basis sites} on which a globally acting extension is primitively specified. Different valid choices of basis are canonically bijective by the same counting-normalization and refactoring invariance principles used throughout the paper, so only the resulting cardinality matters.
\end{defi}

\begin{defi}[Basis coverage and canonical basis minimality]
\label{def:max-density}
An admissible foundational core candidate sealed against $I_n^{(k)}$ has \emph{basis coverage} if every basis site of $I_{n,\mathrm{bas}}^{(k)}$ receives at least one primitive interaction datum from the new extension. It is in \emph{canonical basis-minimal form} if, after elaboration and normalization, each basis site contributes at most one irreducible public trace schema. Equivalently, presentation-equivalent duplicate clauses have been eliminated from the normalized public signature.
\end{defi}
...
\label{thm:coverage}
Let $\B_n$ be a library state in a fully coupled foundation, let $X$ be an admissible foundational core extension sealed against the historical interface $I_n^{(k)}$, and let $I_{n,\mathrm{bas}}^{(k)}$ be a chosen basis for that interface. If the extended theory is to pass the sealed library's global admissibility discipline, then every basis site of $I_{n,\mathrm{bas}}^{(k)}$ must receive at least one primitive interaction datum from~$X$. For native core extensions this is enforced by canonicity-preserving totality of the operational semantics, while for promoted interface packages it is enforced by operational exhaustiveness of the exported global interface.
\end{thm}
...
Let $e$ be an admissible surface extension declaration over~$\B_n$ whose candidate $X(e)$ is sealed against the historical interface $I_n^{(k)}$. Then the normalized public signature $N(e)$ contains at most one irreducible public trace schema for each basis site of $I_{n,\mathrm{bas}}^{(k)}$. Equivalently, redundant primitive clauses that differ only by transparent polymorphic compression, definitional equality, transport, or generic structural operations are eliminable from the canonical normalized presentation.
\end{prop}
...
\begin{thm}[Basis-sensitive density law]
\label{thm:canonicity}
Let $\B_n$ be a library state in a fully coupled foundation, let $X$ be an admissible foundational core extension sealed against the historical interface $I_n^{(k)}$, and let $I_{n,\mathrm{bas}}^{(k)}$ be a chosen basis for that interface. Then
\[
  \Delta(X \mid \B_n) = |I_{n,\mathrm{bas}}^{(k)}|.
\]
\noindent
In particular, sealing cost counts irreducible basis clauses rather than the raw number of exported generators.
\end{thm}
...
\]
If $e_k$ is the declaration whose sealing exports~$L_k$, and if $\mu_k := \mu(e_k)$, then
\[
...
\begin{proof}
By \autoref{thm:adequacy}, the trace part of $N(e)$ records exactly the irreducible public trace schemas arising from resolved obligations of~$X(e)$. By \autoref{prop:canonical-presentation}, $\mu(e)$ is exactly the cardinality of that canonical trace part. By \autoref{thm:canonicity}, there is one and only one irreducible primitive trace schema for each basis site of the active interface, so that cardinality is precisely $\Delta(X(e)\mid\B_n)$. The final formula is then the trace-principle decomposition of \autoref{thm:trace}, rewritten using $\mu_k=\Delta(X(e_k)\mid\B_{k-1})$.
\end{proof}

\begin{rem}[Syntactic scope of the bridge]
\label{rem:bridge-to-counting}
The results of \autoref{thm:elaboration}, \autoref{thm:adequacy}, \autoref{prop:irreducible-necessary}, and \autoref{prop:canonical-presentation} are syntactic bridge theorems: they speak only about elaboration, sealing, opacity, counting normalization, and the presentation-equivalence relation of \autoref{def:mu}. They justify that $N(e)$ and $\mu(e)$ are invariants of explicit extension declarations over the fixed cubical core~$C$.
...
\begin{proof}
Let $\phi$ be a historical trace field of arity $k \geq 3$ appearing in $N_{\mathrm{tr}}(e)$. By \autoref{thm:adequacy}, $\phi$ corresponds to a visible resolved obligation in the public signature of~$e$. By \autoref{lem:horn-reduction}, the additional datum contributed by that arity-$k$ step is a horn-extension fiber $H_k(\partial)$ over a boundary~$\partial$ already exported by lower-arity trace fields. Cubical Kan structure computes a canonical inhabitant of that fiber from the boundary alone: $\hcomp$ synthesizes the missing remote face, $\transp$ aligns the comparison along the relevant boundary transport, and $\mathsf{hfill}$ packages the resulting higher filler.

Replace the primitive witness for~$\phi$ by that synthesized term internally before sealing. The resulting declaration is presentation-equivalent to~$e$, because presentation equivalence already identifies elaborated declarations that differ only by definitional equality, canonical telescope isomorphism, or transparent internal normalization. The public typing judgment is unchanged, but the corresponding trace field is no longer irreducible: it is definitionally reconstructed from lower-arity boundary data and therefore disappears from the canonical normalized trace signature. Since the public trace signature is finite, iterating this elimination over all fields of historical arity at least~$3$ yields a presentation-equivalent declaration~$e'$ with no such irreducible fields. By \autoref{prop:canonical-presentation}, $\mu(e)$ is computed exactly on that canonical trace signature, so no field of arity at least~$3$ contributes to minimal opaque cost.
\end{proof}
...
\]
for every $k \geq 3$, and iterating yields $\Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)$ for all $k \geq 2$. Separately, \autoref{thm:higher-elim} gives the surface-syntactic corollary that any arity-$k \geq 3$ trace witness is definitional shadow of lower-arity boundary data and therefore disappears from a $\mu$-minimal public presentation.
\end{proof}
...
\]
between the two composites from $X$ to $L_{n-2}$, and whose second component is the $3$-cell filling the box once that face is chosen. Cubical Kan filling computes this horn-extension term from the displayed boundary data: $\hcomp$ synthesizes the missing face and $\mathsf{hfill}$ the accompanying filler. Thus the comparison with $L_{n-2}$ factors through the already exported signature of~$L_n$ together with the adjacent bridge carried by~$L_{n-1}$; it does not survive as a new primitive remote field in a $\mu$-minimal presentation.

The same argument now iterates. Suppose inductively that the comparison from $X$ to $L_{n-r+1}$ has already been factored through the recent history. Since $L_{n-r+1}$ was sealed against $L_{n-r}$ and $L_{n-r-1}$, its exported trace again contains the adjacent unary bridge to $L_{n-r}$ together with the corresponding binary square witnessing the comparison of composites. Combining that previously exported square with the already factored comparison for $X$ forms another open $3$-box, so one obtains another horn-extension fiber over already available boundary data. Cubical Kan filling computes that fiber again, yielding a comparison from $X$ to $L_{n-r}$ which factors through the normalized public signature of~$L_{n-r+1}$. Inductively, every remote binary comparison is therefore a definitional consequence of the trace data already exported by the recent history, and \autoref{thm:higher-elim} removes it from any $\mu$-minimal signature as a primitive field.
\end{proof}
...
\autoref{thm:higher-elim}, \autoref{lem:telescopic} & formalized abstractly, read through bridge & \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/ChronologicalWindow.agda} & yes \\
...
On the counting side, the Agda development makes the modeling discipline explicit. The active primitive interface is represented as the disjoint union of the two most recent schema sets, and the saturation step is stated separately rather than hidden in the syntax of imports. In the paper's current basis-sensitive presentation, recurrence counts the corresponding irreducible basis sites of that interface and then reads that abstract count as minimal opaque cost via \autoref{cor:mu-delta}. The concrete decompositions in \texttt{Saturation/Decomposition.agda} then record, step by step, how the next obligation set splits into contributions from the two most recent sealed layers.
