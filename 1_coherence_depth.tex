...
The Cubical Agda development described in \autoref{sec:mechanization} gives exact formal counterparts for \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{thm:higher-elim}, \autoref{lem:telescopic}, \autoref{thm:upper}, \autoref{cor:chrono-window}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}; \autoref{sec:mechanization} records the exact Agda theorem names and the trusted boundary. The dimensional, categorical, and topological arguments remain part of the presentation because they expose the theorem architecture and its examples, while the extension-calculus layer and the main depth claims are also formalized.
...
\]
Equivalently, every admissible declaration is presentation-equivalent to one whose $\mu$-minimal normalized public signature contains only unary and binary primitive historical trace fields. Hence the coherence depth is at most~$2$.
\end{thm}
...
\]
for every $k \geq 3$, and iterating yields $\Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)$ for all $k \geq 2$. The syntactic reformulation follows from \autoref{thm:higher-elim}: the same horn-extension computation shows that any arity-$k \geq 3$ trace witness is definitional shadow of lower-arity boundary data and therefore disappears from a $\mu$-minimal public presentation.
\end{proof}
...
The cubical depth-two theorem can now be separated from the particular implementation mechanism used to prove it. What matters abstractly is only a lower-bound source of genuine binary coherence, a presentation-invariant elimination theorem removing primitive arity-$3$ and higher trace fields from $\mu$-minimal signatures, and a factorization theorem forcing all surviving binary data through the previous two normalized signatures.
...
\begin{enumerate}
  ...
  \[
    \Ocal^{(2)}(X) \not\simeq \Ocal^{(1)}(X).
  \]
  \item For every admissible declaration~$e$ and every $k \geq 3$, the marginal passage from depth $k-1$ to depth $k$ is represented by a horn-extension fiber over $\Ocal^{(k-1)}(X(e))$ whose witness is eliminable from every $\mu$-minimal public signature, and every surviving binary clause factors through a chronological window of size~$2$.
\end{enumerate}
...
\begin{proof}
Property~(2) gives the exact stabilization $\Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)$ for all $k \geq 2$, because the only new data beyond depth~$2$ are horn-extension fibers whose witnesses are eliminated from $\mu$-minimal public signatures, and it also gives a genuine two-layer chronological window by recent-history factorization. Hence \autoref{def:depth} and \autoref{thm:recurrence} imply that the depth is at most~$2$ and that the recurrence is the depth-two affine law. Property~(1) excludes depth~$1$. Hence the least stabilizing depth is exactly~$2$, and substituting $d=2$ and $\kappa_n=c$ into \autoref{thm:recurrence} gives the displayed formula. The Fibonacci shift is exactly the argument of \autoref{cor:fibonacci}.
\end{proof}

Cubical type theory realizes Property~(2) by Kan composition. Two-level type theory offers a different realization of the same architectural pattern: the fibrant equality retains genuine higher path structure, while the strict outer equality supplies a separate level in which higher coherence bookkeeping can be strictified without reintroducing new primitive fibrant trace fields \cite{altenkirch-strict,annenkov-2ltt}. In the sense isolated by \autoref{thm:2d-foundations}, the implementation mechanism proving stabilization may differ, but the metatheoretic consequence does not: once a foundation has genuine binary coherence together with a uniform rule eliminating primitive arity-$3$ trace fields from $\mu$-minimal signatures and forcing recent-history factorization, the worst-case recurrence is forced to be the same depth-two affine law, and therefore the same Fibonacci shift under constant payload.
...
The formalization is organized around the fixed extension calculus of \autoref{sec:framework}, its depth theorems, the induced recurrence law, and the clutching construction. This section records the exact paper-to-Agda theorem map for that theorem package.
...
The formalization establishes the extension-calculus theorem package and the cubical witness results on which it rests. The exact paper-to-Agda correspondences are listed above; the cited modules use no \texttt{postulate}; and the clutching module is checked separately only because its HIT imports live on the Cubical library's standard natural-number interface rather than on \path{Core/Nat.agda}. The mathematical arguments of \autoref{sec:coherence} remain important as explanation and as links to the categorical and topological examples, and the main claims about the fixed cubical extension calculus are formalized as well. What remains at the paper level is the bridge from those formal metatheorems to the normalized-signature quantity~$\mu$.
\end{rem}
