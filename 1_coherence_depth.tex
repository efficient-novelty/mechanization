...
One semantic clarification is important before the cubical argument begins. In univalent foundations, types behave as weak $\omega$-groupoids, so the geometric space of fillers for a fixed higher boundary is not generally contractible; open $3$-boxes can admit many fillers. The integration-latency invariant does not count that homotopy-theoretic space. It counts the burden of supplying new primitive, human-authored, domain-specific payload at sealing time. In cubical type theory, the operational semantics provide the primitive $\hcomp$, which computes a specific syntactic filler for any well-typed open box. The claim below is therefore algorithmic rather than topological: arity-$3$ obligations introduce no new primitive payload because the calculus uniformly synthesizes one filler from already exported unary and binary boundary data.
...
Third, arity at least~$3$ contributes no new primitive payload. Identity types in intensional type theory carry weak $\omega$-groupoid structure~\cite{lumsdaine,vdberg-garner}, so one should not expect geometric uniqueness of higher fillers in general. However, the cubical operational presentation equips the calculus with Kan operations $\hcomp$, $\compop$, and $\transp$ that compute a specific filler term for any well-typed open box from its lower-dimensional boundary data~\cite{cubical,cchm-hits}. Therefore any apparent arity-$k$ obligation with $k \geq 3$ is algorithmically synthesized from its unary and binary boundary trace and contributes no new primitive clause to sealing. No irreducible independent obligation survives beyond arity~$2$.
\end{proof}
...
\]
between the two composites from $X$ to $L_{n-2}$. Cubical Kan filling via $\hcomp$ computes a particular syntactic filler for that box, so $\gamma_{n-2}$ is derived rather than primitive.
