...
\]
under constant payload size~$c$; after the shift $U_n=\mu_n+2c$, the sequence is Fibonacci. The accompanying Cubical Agda development formalizes the candidate-and-obligation calculus, the cubical upper/lower metatheory, and the abstract recurrence package; \autoref{sec:mechanization} records the exact paper-to-Agda map, the trusted boundary, and where the passage to the surface-syntactic quantity~$\mu$ still uses the paper-level bridge.
\end{abstract}
...
The Cubical Agda development described in \autoref{sec:mechanization} formalizes the abstract candidate-and-obligation calculus, the cubical upper/lower metatheory, the contractible-factor decomposition of higher exact fibers, the recent-history factorization chain, and the abstract recurrence package underlying \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{thm:upper}, \autoref{cor:contractible-factor}, \autoref{cor:chrono-window}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}. The surface-syntactic readings of \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:factorization-complete}, \autoref{thm:higher-elim}, \autoref{lem:telescopic}, and the $\mu$-based recurrence are then obtained in the paper by combining those formal metatheorems with the bridge results of \autoref{sec:bridge}. \autoref{sec:mechanization} records the exact Agda theorem names and the trusted boundary.
...
\autoref{thm:higher-elim} & elimination from $\mu$-minimal normalized public signatures & \autoref{lem:horn-reduction}, \autoref{thm:computational-replacement}, and canonical presentation & bridge corollary \\
...
\begin{thm}[Computational replacement preserves canonical presentation]
\label{thm:computational-replacement}
Let $e$ be an admissible surface extension over~$\B_n$, and let $\phi$ be a primitive historical trace clause in the elaborated sealed export of~$e$. Suppose there is a lower-arity boundary subfamily $\partial_\phi$ already exported by the remaining trace clauses and a cubically synthesized term
\[
  t_\phi(\partial_\phi),
\]
built only from $\hcomp$, $\transp$, $\mathsf{hfill}$, definitional equality, and the transparent structural operations already quotiented by counting normalization, such that the clause witnessing~$\phi$ is definitionally equal in the sealed typing judgment to~$t_\phi(\partial_\phi)$. Let $e[\phi \rightsquigarrow t_\phi]$ be the declaration obtained by replacing that primitive clause by the synthesized term before sealing. Then:
\begin{enumerate}
  \item $e[\phi \rightsquigarrow t_\phi] \approx e$;
  \item in the canonical normalized trace signature of the resulting presentation-equivalence class, the field corresponding to~$\phi$ is transparently derived from the remaining boundary data and therefore absent as an irreducible primitive trace field;
  \item every other irreducible payload or trace field corresponds canonically to one from~$N(e)$, so the replacement introduces no new irreducible public fields.
\end{enumerate}
\end{thm}

\begin{proof}
By hypothesis, replacing the primitive clause for~$\phi$ by $t_\phi(\partial_\phi)$ changes the elaborated sealed declaration only by definitional equality and transparent internal normalization inside the same public typing judgment. Therefore $e[\phi \rightsquigarrow t_\phi] \approx e$ by \autoref{def:mu}. Because the replacement term is built only from lower-arity boundary data and the transparent cubical operations already quotiented by \autoref{def:atomic-schema}, the corresponding field is no longer primitively inhabited: it is uniformly reconstructed from the remaining exported trace data.

Now apply \autoref{thm:adequacy}. Counting normalization introduces no new primitive public datum, and it discards fields that have become transparently synthesized from earlier ones. Hence the field corresponding to~$\phi$ does not survive as an irreducible element of the canonical normalized trace signature of the presentation-equivalence class. Every other irreducible payload or trace field belongs to the same sealed typing judgment and therefore corresponds canonically under \autoref{prop:canonical-presentation}. Thus the only effect of the replacement is to move~$\phi$ from primitive syntax into transparent derivation, with no new irreducible public field introduced elsewhere.
\end{proof}

\begin{rem}[Status of the replacement bridge]
\label{rem:replacement-bridge}
\autoref{thm:computational-replacement} is presently the main remaining paper-level bridge lemma. The Agda development formalizes the horn-extension fiber and its canonical cubical inhabitant, but not yet the passage from that synthesized witness to a theorem about surface declarations, presentation equivalence, and elimination from canonical normalized public signatures.
\end{rem}
...
Apply \autoref{thm:computational-replacement} to replace the primitive clause~$\phi$ by that synthesized witness before sealing. The theorem shows that this changes only the source presentation: in the canonical normalized trace signature of the presentation-equivalence class, the field corresponding to~$\phi$ is transparently derived from lower-arity boundary data and contributes no irreducible primitive trace schema. Since any surface declaration contains only finitely many primitive clauses of arity at least~$3$, iterating this replacement yields a presentation-equivalent declaration~$e'$ with no such primitive clauses left. By \autoref{prop:canonical-presentation}, the same canonical normalized trace signature is attached to every representative of that class, so it contains no irreducible historical field of arity~$k \geq 3$. That is exactly the stated elimination claim, and it is on that canonical signature that~$\mu(e)$ is computed.
\end{proof}
...
\midrule
Chosen CCHM-style cubical core / Cubical Agda realization & yes; explicit swap and clutching witnesses & yes; horn reduction, stabilization, and recent-history factorization & yes, with \autoref{thm:computational-replacement} still paper-level & yes \\
...
The formalization is organized around the fixed extension calculus of \autoref{sec:framework}, its depth theorems, the induced recurrence law, and the clutching construction. \autoref{tab:mechanization-status} first makes the trusted boundary local. Here \emph{fully formalized} means that the paper's statement is realized directly in the cited Agda modules; \emph{formalized abstractly, read through bridge} means that the Agda development proves the structural theorem and the paper reads it through the normalization and canonical-presentation results of \autoref{sec:bridge}; and \emph{paper-level only} marks the bridge layer itself. In particular, \autoref{thm:computational-replacement} is the main remaining paper-level bridge lemma: the Agda development proves the cubical synthesis of the replacement witness, but not yet its passage through presentation equivalence and counting normalization. The itemized map after the table then records the exact paper-to-Agda theorem correspondence.
...
\autoref{thm:computational-replacement} & paper-level only & none & n/a \\
...
\autoref{thm:factorization-complete}, \autoref{thm:higher-elim}, \autoref{lem:telescopic} & formalized abstractly, read through bridge & \path{Metatheory/KanSubsumption.agda}, \path{Metatheory/UpperBound.agda}, \path{Metatheory/ChronologicalWindow.agda} & yes \\
...
\autoref{thm:2d-foundations} & fully formalized & \path{Metatheory/TwoDFoundations.agda} & no \\
...
\item \autoref{thm:computational-replacement} is presently paper-level only. It packages the bridge claim that replacing a primitive higher-arity trace clause by the cubically synthesized witness preserves presentation equivalence and removes that clause from irreducible canonical normalized public form. The current Agda development proves the synthesis side via \path{HornExtensionFiber}, \path{structural-integration-horn-reduction}, and \path{remote-layer-obligation-derived}, but not yet that normalization-and-presentation theorem itself.
...
\item \autoref{thm:higher-elim} is the paper's syntactic corollary of \path{Metatheory/KanSubsumption.agda} and \path{Metatheory/UpperBound.agda}: the Agda development formalizes the horn-extension fiber, its canonical inhabitant, and stabilization at depth~$2$ via \path{HornExtensionFiber}, \path{structural-integration-horn-reduction}, \path{remote-layer-obligation-derived}, \path{structural-obligation-set-equivalence}, and \path{structural-stabilizes-at-two}; the elimination of primitive arity-$3$ fields from $\mu$-minimal public signatures is obtained in the paper by combining those results with \autoref{thm:computational-replacement} and the rest of the bridge normalization theorems of \autoref{sec:bridge}.
...
\item \autoref{thm:2d-foundations} corresponds exactly to \path{Metatheory/TwoDFoundations.agda}. The weaker primitive/window layer is packaged there by \path{PrimitiveWindow2DFoundation}, \path{primitive-depth-two-law-for-2d-foundations}, \path{chronological-window-size-two-for-2d-foundations}, \path{cubical-primitive-window-2d-foundation}, and \path{cubical-primitive-depth-two-law-for-2d-foundations}; the separate exact-depth wrapper of the theorem is packaged by \path{FullyCoupled2DFoundation}, \path{depth-two-law-for-2d-foundations}, and \path{cubical-depth-two-law-for-2d-foundations}; \path{constant-payload-depth-two-law} supplies the payload-aware arithmetic specialization.
...
The formalization includes both direct cubical metatheory and a distilled extension calculus. The UIP collapse, horn-extension computation, recent-history factorization chain, lower-bound obstruction, exact depth-two corollary, and clutching-family witness are mechanized against cubical or UIP primitives themselves. The surface-syntactic theorems about normalized signatures and minimal opaque cost are then obtained in the paper by combining those formal metatheorems with the bridge normalization results of \autoref{sec:bridge}. The canonicity, trace, refactoring, recurrence, and abstract $2$D-foundations layers package those results as the sealed-interface and counting calculus studied in the paper. The formalized core is therefore explicit, but the passage to the paper's $\mu$-based syntax still uses the bridge.
