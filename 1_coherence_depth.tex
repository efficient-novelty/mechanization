...
The Cubical Agda development described in \autoref{sec:mechanization} formalizes the abstract candidate-and-obligation calculus, the cubical exact upper- and lower-bound metatheory, the contractible-factor decomposition of higher exact fibers, the recent-history factorization chain, the canonical-trace replacement interface for $\mu$-minimal elimination, and the abstract recurrence package underlying \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{thm:upper}, \autoref{cor:contractible-factor}, \autoref{cor:chrono-window}, \autoref{thm:horn-interface}, \autoref{cor:cubical-horn-interface}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}. The remaining surface-syntactic readings of \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:factorization-complete}, \autoref{thm:higher-elim}, \autoref{lem:telescopic}, and the $\mu$-based recurrence are then obtained in the paper by combining those formal metatheorems with the bridge results of \autoref{sec:bridge}. \autoref{sec:mechanization} records the exact Agda theorem names and the trusted boundary.
...
\autoref{thm:computational-replacement} is formalized on the explicit canonical-trace-presentation interface: \path{Metatheory/ComputationalReplacement.agda} makes presentation equivalence, primitive-versus-derived tagging, and elimination from the $\mu$-minimal part of a canonical trace signature explicit. The paper-level part of \autoref{sec:bridge} is the surface-to-interface normalization passage from raw surface declarations to that canonical presentation interface via elaboration soundness, adequacy, and canonical normalized presentation.
\end{rem}
...
This is an elimination statement about $\mu$-minimal normalized public signatures, not a claim that the corresponding exact obligation types are absent from $\Ocal^{(k)}(X)$. The cubical operations synthesize the replacement witness; the bridge theorem \autoref{thm:computational-replacement} upgrades that witness to a presentation-equivalent minimal signature.
\end{thm}
...
\midrule
Chosen CCHM-style cubical core / Cubical Agda realization & yes; explicit swap and clutching witnesses & yes; horn reduction, stabilization, and recent-history factorization & yes; replacement interface formalized, surface-to-interface normalization paper-level & yes \\
...
The formalization is organized around the fixed extension calculus of \autoref{sec:framework}, its horn-computational abstraction layer, its depth theorems, the induced recurrence law, and the clutching construction. \autoref{tab:mechanization-status} first makes the trusted boundary local. Here \emph{fully formalized} means that the paper's statement is realized directly in the cited Agda modules; \emph{formalized abstractly, read through bridge} means that the Agda development proves the structural theorem and the paper reads it through the normalization and canonical-presentation results of \autoref{sec:bridge}; and \emph{paper-level only} marks the bridge layer itself. In particular, \autoref{thm:computational-replacement} is fully formalized on explicit canonical trace presentations. The paper-level part of the bridge is the surface-to-interface normalization package of \autoref{sec:bridge}, namely the path from raw declarations to that canonical presentation interface. The itemized map after the table records the exact paper-to-Agda theorem correspondence.
