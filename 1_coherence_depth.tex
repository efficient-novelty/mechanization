...
\autoref{thm:recurrence} & affine recurrence for $\mu$ & \autoref{thm:trace}, \autoref{cor:mu-delta}, and the exact depth/window package & formalized abstractly, read through bridge \\
...
\item \autoref{thm:recurrence} corresponds exactly to \path{Metatheory/UniversalRecurrence.agda} via \path{UniversalAffineRecurrence} and \path{universal-affine-recurrence}, read through \autoref{cor:mu-delta} so that the paper's surface-syntactic recurrence is stated over~$\mu$.
