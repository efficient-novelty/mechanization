...
\end{itemize}
The metatheory modules implement the semantic dictionary used throughout the paper. In the extensional/UIP case an obligation contributes no new primitive cost because its semantic fiber collapses over the unary boundary. In the cubical case the normalized depth-three datum is an open-box extension problem rather than a closed-sphere filler problem, so the relevant notion is computational: an obligation contributes no new primitive cost when a uniform \texttt{hcomp}/\texttt{hfill}-style derivation synthesizes the missing face and filler from already exported boundary data.
...
\end{itemize}
This is the mechanized form of the telescopic subsumption claim: once the visible unary and binary faces are fixed, cubical composition uniformly computes a witness for the missing arity-$3$ face and the accompanying filler. This is exactly the computational ingredient used in the paper's strict obligation-set equivalence after normalization as a horn extension. The mechanization does not assert that the filler space of a fully closed geometric boundary is contractible; it asserts that the open-box extension problem relevant to sealing is algorithmically solved by the ambient cubical operations.
...
The mechanism behind the recurrence is structural. For globally acting foundational extensions, totality and exhaustiveness in the foundational-core regime force maximal interface density. Sealing cost is therefore identified with the size of the active interface. The Integration Trace Principle then follows from the dependent-record form of opaque exports: resolved coherence data disappear as derivations but remain present in the public signature as trace fields. In the cubical univalent case, the marginal higher-arity data are horn extensions rather than closed-sphere fillers. Those extension types are canonically contractible. Telescopic subsumption therefore shows that all older comparisons are algorithmically synthesized from the adjacent exported traces, so the primitive active interface is generated by the previous two layers; under constant payload size this yields an affine depth-two recurrence whose shifted form is Fibonacci.
