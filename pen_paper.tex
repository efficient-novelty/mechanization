...
\Cref{sec:verification} describes the computational verification: the Haskell engine, the strict molecular evaluation loop, auxiliary diagnostics, rejected candidates, and Cubical Agda mechanization.
...
The manuscript canon is fixed by exact-band molecular compilation together with hybrid strict evaluation:
\[
...
This is the only Step-15 value used in the manuscript.
...
In this manuscript that principle is realized operationally by molecular compilation plus the strict hybrid evaluator.
...
\[
  \nu(\mathrm{DCT}) = 88.
\]
That value is produced by the current hybrid strict evaluator from the conceptual temporal shell plus any explicit topological witness payload preserved by compilation, and it is the only theorem-level DCT novelty value used in this manuscript.
...
\begin{enumerate}[nosep, start=4]
    ...
    \item \textbf{Mechanical Verification.} The strict bounded recovery engine recovers the full 15-step sequence with the machine values reported in \cref{tab:genesis}. Auxiliary audits validate portions of that computation but do not override those totals.
\end{enumerate}
...
In the current manuscript that principle is realized through molecular compilation and strict evaluation.
...
No finer DCT novelty total is elevated to theorem status unless it is exposed by the current evaluator.
