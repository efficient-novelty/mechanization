...
% ============================================
% ============================================
% SECTION 6: COMBINATORIAL NOVELTY
% ============================================
...
We prove that in constructive type theory, novelty grows \emph{superlinearly}: exponentially for ordinary inductive types, and---through dependent elimination and library cross-interaction---superlinearly for higher inductive types as well.

The argument proceeds in four steps: we establish exponentiality for OITs (\S\ref{sec:oit}), identify the constraint that path constructors impose on HITs (\S\ref{sec:hit-constraint}), show how dependent elimination and library growth restore superlinear scaling (\S\ref{sec:library-novelty}), and prove that efficiency diverges (\S\ref{sec:escape}).
...
\begin{corollary}[OIT Product Exponentiality]
\label{cor:oit-product}
If $X$ and $Y$ are OITs with $\Delta_0^X$ and $\Delta_0^Y$ point constructors respectively, the product type $X \times Y$ admits $2^{\Delta_0^X + \Delta_0^Y}$ maps to $\mathbf{2}$.
In general, for OITs, novelty is \emph{multiplicative} under products: $\nu(X \times Y) \geq \nu(X) \cdot \nu(Y)$.
\end{corollary}

\begin{proof}
The product $X \times Y$ is an OIT with $\Delta_0^X \cdot \Delta_0^Y$ point constructors (one per pair).
A map $X \times Y \to \mathbf{2}$ chooses a Boolean for each pair, giving $2^{\Delta_0^X \cdot \Delta_0^Y} \geq 2^{\Delta_0^X} \cdot 2^{\Delta_0^Y}$.
\end{proof}

\subsection{HIT Constraints on Non-Dependent Elimination}
\label{sec:hit-constraint}

\begin{remark}[HIT Constraints on Boolean Predicates]
\label{rem:hit-constraints}
For a higher inductive type $X$, path constructors impose coherence on eliminators into \emph{sets} (0-truncated types).
A function $f : X \to \mathbf{2}$ must map path-connected points to the same value (since $\mathbf{2}$ is discrete).
The number of such functions equals $2^{|\pi_0(X)|}$, the number of connected components.
\end{remark}

\begin{example}[Concrete HIT constraints]
\label{ex:hit-constraints}
\begin{itemize}[nosep]
    \item $S^1$ has $\Delta = 2$ (one point, one loop), $\pi_0 \cong 1$: only $2^1 = 2$ maps to~$\mathbf{2}$ (both constant), not $2^2 = 4$.
    \item The torus $T^2$ has $\Delta = 4$ (one point, two loops, one surface), $\pi_0 \cong 1$: only $2^1 = 2$ maps to~$\mathbf{2}$, not $2^4 = 16$.
    \item The wedge $S^1 \vee S^1$ has $\Delta = 3$ (one point, two loops), $\pi_0 \cong 1$: only $2^1 = 2$ maps to~$\mathbf{2}$, not $2^3 = 8$.
\end{itemize}
Thus OIT exponentiality does not extend to HITs when the codomain is a set.
The path constructors ``absorb'' the degrees of freedom of the point constructors they connect.
\end{example}

\subsection{Dependent Elimination and Library Novelty}
\label{sec:library-novelty}

The preceding section shows that \emph{non-dependent} elimination of HITs into sets is constrained.
However, the novelty measure (\cref{def:novelty}) counts \emph{all} newly enabled constructions, not just maps to $\mathbf{2}$.
Three mechanisms restore superlinear growth.

\subsubsection{Mechanism 1: Dependent Elimination}

The eliminator of a HIT $X$ into a \emph{type family} $P : X \to \U$ is far richer than elimination into a fixed type.
For each point constructor $c$, we choose $P(c) : \U$ (a type, not just a term).
For each path constructor $p : a =_X b$, we must provide a \emph{transport equivalence} $P(a) \simeq P(b)$.

\begin{proposition}[Dependent Elimination Recovers Full Sensitivity]
\label{prop:dep-elim}
Let $X$ be a HIT with $\Delta_0$ point constructors and $\Delta_1$ path constructors, introduced into a library $\mathcal{B}$ containing $|\mathcal{B}|$ types.
The number of dependent type families $P : X \to \U$ with $P(c_i) \in \mathcal{B}$ is at least:
\begin{equation}
    \label{eq:dep-elim-count}
    |\{P : X \to \U\}| \geq |\mathcal{B}|^{\Delta_0} \cdot \prod_{j=1}^{\Delta_1} |\mathrm{Aut}(P(s_j))|
\end{equation}
where $s_j$ and $t_j$ are the source and target of the $j$-th path constructor (which may coincide), and $\mathrm{Aut}(T)$ is the automorphism group of type $T$ in $\mathcal{B}$.
\end{proposition}

\begin{proof}
Each point constructor $c_i$ independently selects its fiber $P(c_i)$ from $\mathcal{B}$, giving $|\mathcal{B}|^{\Delta_0}$ choices.
Each path constructor $p_j : c_{s_j} =_X c_{t_j}$ requires a transport equivalence $P(c_{s_j}) \simeq P(c_{t_j})$.
When $c_{s_j} = c_{t_j}$ (a loop), this is an automorphism of $P(c_{s_j})$.
Different automorphisms yield genuinely different type families (and thus different sections, fibrations, and derived constructions).
\end{proof}

\begin{example}[Circle recovers richness]
\label{ex:circle-dep}
For $S^1$ with $\mathsf{base}$ and $\mathsf{loop}$: a dependent type family $P : S^1 \to \U$ requires $P(\mathsf{base}) = T$ (one choice from $\mathcal{B}$) and $\mathsf{loop}_* : T \simeq T$ (an automorphism of $T$).
If $T = \N$ with successor as automorphism, we get the universal cover.
If $T$ is a finite type with $k$ elements, $|\mathrm{Aut}(T)| = k!$.
The Genesis Table gives $\nu(S^1) = 7$ when $|\mathcal{B}| = 4$; this is consistent with the count of independent dependent-elimination constructions (covering spaces, loop spaces, winding numbers, etc.)\ exceeding the mere $2^1 = 2$ maps to $\mathbf{2}$.
\end{example}

\subsubsection{Mechanism 2: Library Cross-Interaction}

\begin{proposition}[Library Scaling]
\label{prop:library-scaling}
Let $X$ be any type (OIT or HIT) introduced at step $n+1$ into a library $\mathcal{B}_n$ with $|\mathcal{B}_n| = n$ types.
The number of newly enabled constructions involving $X$ and the library grows at least as $\Omega(n)$.
\end{proposition}

\begin{proof}
For each library type $T \in \mathcal{B}_n$, the introduction of $X$ enables at minimum:
\begin{enumerate}[nosep]
    \item The function type $X \to T$ (a new type, with its own eliminator).
    \item The product type $X \times T$ (enabling projections, pattern-matching).
    \item The dependent sum $\Sigma_{x:X} P(x)$ for type families $P$ with fibers in $\{T\}$.
\end{enumerate}
Each of these is a distinct schema (by type-theoretic distinctness of $T$), contributing at least $3n$ new constructions.
More precisely, the function types alone contribute: for each $T$, the space of maps $X \to T$ has cardinality $|T|^{\Delta_0}$ (for OITs) or $|T|^{|\pi_0(X)|}$ (for HITs into sets), each a distinct construction.
\end{proof}

\begin{remark}
This mechanism explains why the Genesis Sequence's $\nu$ values grow steadily even for HITs: at step $n$, there are $n - 1$ prior types to interact with.
The library is both the source of novelty (new cross-constructions) and the sink that raises the bar (cumulative efficiency).
The key structural fact is that the library grows \emph{linearly} in $n$ (one type per realization), while the bar grows \emph{sub-linearly} (as a cumulative average).
\end{remark}

\subsubsection{Mechanism 3: Composite Constructions}

\begin{proposition}[Composite Novelty is Superadditive]
\label{prop:composite}
Let $X, Y \in \mathcal{B}$ be types with novelties $\nu_X, \nu_Y$ respectively.
The composite types $X \times Y$ and $X \to Y$ contribute novelty at least \emph{additive} in $\nu_X$ and $\nu_Y$:
\begin{equation}
    \nu(X \times Y \mid \mathcal{B}) \geq \nu_X + \nu_Y
\end{equation}
For OITs, the bound is multiplicative: $\nu(X \times Y \mid \mathcal{B}) \geq \nu_X \cdot \nu_Y$ (\cref{cor:oit-product}).
\end{proposition}

\begin{proof}
The product $X \times Y$ admits all constructions of the form $f \circ \pi_1$ (lifting an $X$-construction) and $g \circ \pi_2$ (lifting a $Y$-construction), giving at least $\nu_X + \nu_Y$ distinct constructions.
For OITs, the stronger multiplicative bound follows from \cref{cor:oit-product}.
For HITs, the path structure of $X \times Y$ may impose additional constraints, reducing the count below $\nu_X \cdot \nu_Y$ but preserving the additive bound (since projections respect path structure).
\end{proof}

\subsection{Asymptotic Escape Velocity}
\label{sec:escape}

We now prove the main dynamical result: efficiency diverges.
The argument requires one structural observation about Construction Effort.

\begin{lemma}[Bounded Effort]
\label{lem:bounded-effort}
In the PEN model, the Construction Effort $\kappa_n$ of the $n$-th realization is bounded above by a constant $K$ independent of $n$.
\end{lemma}

\begin{proof}
Each mathematical structure in the Genesis Sequence is specified by a \emph{fixed finite presentation}: a finite list of constructors (type formers, point constructors, path constructors).
This presentation does not grow with the library.
The circle always has 2 constructors ($\mathsf{base}$, $\mathsf{loop}$) regardless of whether the library contains 4 or 400 types.
Cohesion always requires 4 modalities ($\flat$, $\sharp$, $\Pi$, $\mathrm{Disc}$) regardless of how many types they act on.

More formally: by the Horizon Policy (\cref{ax:horizon}), the effort horizon $H$ resets to 2 after each realization and increments by 1 per idle tick.
For a structure to be admissible (\cref{ax:admissibility}), $\kappa(X) \leq H$.
Since $H$ grows from 2 by unit steps, the effort at realization is bounded by $2 + (\text{idle ticks before realization})$.
In the Genesis Sequence, the maximum idle gap before any realization is bounded (empirically, $\kappa_n \leq 9$ for all 15 structures).

For the asymptotic argument, the essential point is that $\kappa$ measures \emph{definitional} complexity (how many generators to write down), not \emph{contextual} complexity (how many library interactions to witness).
The latter is captured by $\Delta$, which grows as $F_n$.
Definitional complexity is a property of the structure's presentation, which is finite and library-independent.
\end{proof}

\begin{remark}
\label{rem:effort-vs-latency}
The distinction between $\kappa$ and $\Delta$ is central to the PEN model.
Integration Latency $\Delta$ grows with the library (Fibonacci) because it counts \emph{coherence obligations} against the accumulated history.
Construction Effort $\kappa$ does not grow with the library because it counts \emph{generators in the presentation}.
Efficiency $\rho = \nu/\kappa$ has a bounded denominator and a growing numerator---this is the structural reason for divergence.
\end{remark}
...
\begin{proof}
\textbf{Step 1 (Novelty grows at least linearly).}
By \cref{prop:library-scaling}, the novelty of the $n$-th realization satisfies $\nu_n \geq c \cdot n$ for some constant $c > 0$ (at minimum, one new construction per prior library type).
Dependent elimination (\cref{prop:dep-elim}) and composite constructions (\cref{prop:composite}) can only increase this bound.

\textbf{Step 2 (Effort is bounded).}
By \cref{lem:bounded-effort}, $\kappa_n \leq K$ for some constant $K$ independent of $n$.

\textbf{Step 3 (Efficiency diverges).}
\begin{equation}
    \rho_n = \frac{\nu_n}{\kappa_n} \geq \frac{c \cdot n}{K} \to \infty
\end{equation}

\textbf{Step 4 (Efficiency outpaces the bar).}
The bar $\mathrm{Bar}_n = \Phi_n \cdot \Omega_{n-1}$, where $\Phi_n \to \varphi$ (a constant) and $\Omega_{n-1} = \sum_{i=1}^{n-1} \nu_i / \sum_{i=1}^{n-1} \kappa_i$ is a cumulative average.

Since $\Omega_{n-1}$ is an average of past efficiencies, and averages grow strictly slower than their fastest-growing terms:
\begin{equation}
    \Omega_{n-1} = \frac{\sum \nu_i}{\sum \kappa_i} \leq \frac{n \cdot \max_i \nu_i}{n \cdot \min_i \kappa_i} = \frac{\max_i \nu_i}{\min_i \kappa_i}
\end{equation}
More precisely, if $\rho_i$ grows at least linearly, then $\Omega_{n-1}$ grows at most as the average of a linear sequence, which is itself linear: $\Omega_{n-1} = O(n)$.
But $\rho_n = \Omega(n)$ as well, so $\rho_n / \mathrm{Bar}_n = \rho_n / (\varphi \cdot \Omega_{n-1})$ does not converge to zero.

In fact, the situation is stronger.
The bar at step $n$ reflects the \emph{average} performance of all prior steps, while $\rho_n$ reflects the \emph{current} performance.
Since novelty grows with the library while effort is bounded, each new step is more efficient than the average of all prior steps.
Formally: if $\rho_n \geq cn/K$, then
\begin{equation}
    \Omega_{n-1} = \frac{\sum_{i=1}^{n-1} \nu_i}{\sum_{i=1}^{n-1} \kappa_i}
    \leq \frac{\sum_{i=1}^{n-1} \nu_i}{n-1}
    \leq \frac{c \cdot \binom{n}{2} / 1}{n-1}
    = \frac{cn}{2}
\end{equation}
where the last step uses $\sum_{i=1}^{n-1} i = \binom{n}{2}$ as an upper bound on $\sum \nu_i$ (which is loose).
Therefore $\mathrm{Bar}_n = \varphi \cdot \Omega_{n-1} \leq \varphi c n / 2$, while $\rho_n \geq cn/K$.
The ratio
\begin{equation}
    \frac{\rho_n}{\mathrm{Bar}_n} \geq \frac{cn/K}{\varphi cn/2} = \frac{2}{\varphi K}
\end{equation}
is a positive constant.
Since $K$ is a small constant (empirically $K \leq 9$) and $2/(\varphi K) > 0$, efficiency permanently clears the bar by a constant factor.
As OIT exponentiality (\cref{thm:oit-exponentiality}), dependent elimination (\cref{prop:dep-elim}), and synthesis (\cref{prop:composite}) further amplify $\nu_n$ beyond the linear lower bound, the actual ratio $\rho_n / \mathrm{Bar}_n$ grows without bound.
\end{proof}

\begin{remark}[Conservative vs.\ actual scaling]
\label{rem:conservative}
\Cref{thm:divergence} uses only the weakest mechanism (library cross-interaction, linear in $n$) and ignores OIT exponentiality and synthesis amplification.
The actual novelty values in the Genesis Table grow faster than linearly: $\nu$ increases from 1 to 150 over 15 steps, with the DCT contributing $\nu = 150$ via the Lattice Tensor Product.
The conservative linear bound establishes that stagnation is \emph{impossible}; the actual dynamics are richer.
\end{remark}

\begin{remark}[The two growth rates]
\label{rem:two-rates}
The PEN model exhibits two distinct growth regimes:
\begin{itemize}[nosep]
    \item \textbf{Integration Latency} $\Delta_n \sim \varphi^n$: exponential, governed by the Fibonacci recurrence.
    \item \textbf{Novelty} $\nu_n$: at least linear in $n$ (from library interactions), potentially much faster (from dependent elimination and synthesis).
\end{itemize}
These operate on different axes: $\Delta$ measures the \emph{cost of coherence} (time), while $\nu$ measures the \emph{payoff in combinatorial power} (capability).
Efficiency $\rho = \nu/\kappa$ is the payoff per unit of \emph{definitional} cost, and since $\kappa$ is bounded while $\nu$ grows, the system never stagnates.
The system pays exponentially growing temporal costs ($\Delta_n$) but receives at-least-linearly growing capability ($\nu_n$) at bounded definitional cost ($\kappa_n$).
\end{remark}
