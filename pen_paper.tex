...
At the \emph{executable layer}, an open-source Haskell synthesis engine operating over a fixed Minimal Binary Type Theory (MBTT) grammar recovers a deterministic 15-step trajectory in its current strict unguided lane. At the \emph{semantic-interpretation layer}, labels such as cohesion, metric, Hilbert, or DCT are post-hoc readings of selected AST skeletons rather than targets encoded in the search. Under that interpretation, the sequence bootstraps minimal type theory, ascends through core homotopy theory, reaches modal and differential API shells, and halts at a temporal-cohesive fixed point whose semantic completion can be read as the Dynamical Cohesive Topos (DCT). The machine-verified strict canon reported here is therefore the output of the present executable artifact under a fixed shared vocabulary, not a hand-calibrated reconstruction or a claim that the system invented its own semantics from scratch.
\end{abstract}
...
\emph{Executable finding:} the current strict unguided Haskell artifact recovers a specific 15-step trajectory under bounded search.
...
Our current implementation addresses this by making MBTT-first typed AST synthesis the default runtime path, so the primary search is syntactic rather than reference-replay.
...
The search also uses a fixed shared vocabulary rather than an unbiased empty grammar; what is absent is target-conditioned scoring, theorem injection, and semantic rewards for any particular modal or geometric label.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{What PEN does not derive:} A semantic theorem that every selected AST already contains the full body of the human mathematical concept named in the ``Structure'' column.
    \item \textbf{What the implementation establishes:} Recovery of this trajectory in the current strict unguided lane, not exhaustive search over the full admissible MBTT space.
\end{enumerate}
...
A natural and mathematically precise objection to the Step~13 metric package asks how the specification can formulate a symmetric bilinear form $g : TX \otimes_s TX \to \R$ without paying a large scalar penalty $\kappa_{\mathrm{scalar}}$ to discretely axiomatize the continuum and its commutative ring structure.
...
A direct inspection of the machine-verified MBTT AST for the metric bundle (\cref{tab:step13}) shows that the selected syntax for the metric tensor is
\[
